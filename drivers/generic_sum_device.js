...
class SumMeterDevice extends Device {
  ...
  async updateMeter(val) { // , pollTm) { // pollTm is lastUpdated when using pollMethod
    try {
      ...
      if ((lastVal !== null) && (meterDelta > 10000)) throw Error('ignoring uneralistic incoming meter value!', value);
  ...
  async minMaxReset(reset, source) {
    ...
    await this.setCapability('measure_watt_max', this.lastMinMax.wattMax).catch(this.error);
    await this.setCapability('measure_lpm_max', this.lastMinMax.lpmMax).catch(this.error);
    await this.setCapability('measure_watt_min', this.lastMinMax.wattMin).catch(this.error);
    await this.setCapability('measure_lpm_min', this.lastMinMax.lpmMin).catch(this.error);
    await this.setCapability('last_minmax_reset', `${date} ${time}`).catch(this.error);
    await this.setStoreValue('lastMinMax', this.lastMinMax).catch(this.error);
  }

  async updateMeters({ ...reading }, { ...periods }) {
    await this.setCapability(this.ds.cmap.meter_source, reading.meterValue).catch(this.error);
    ...
    if (periods.newHour) {
      // new hour started
      await this.setCapability(this.ds.cmap.last_hour, valHour).catch(this.error);
    ...
    if (periods.newDay) {
      // new day started
      await this.setCapability(this.ds.cmap.last_day, valDay).catch(this.error);
    ...
    if (periods.newMonth) {
      // new month started
      await this.setCapability(this.ds.cmap.last_month, valMonth).catch(this.error);
    ...
    if (periods.newYear) {
      // new year started
      await this.setCapability(this.ds.cmap.last_year, valYear).catch(this.error);
    ...
    await this.setCapability(this.ds.cmap.this_year, valYear).catch(this.error);
  ...
  async updateTargets({ ...periods }) {
    ...
    if (this.budgets && this.budgets.yearToDate) {
      ...
      await this.setCapability('meter_target_year_to_date', onTarget).catch(this.error);
    }
    if (this.budgets && this.budgets.monthToDate) {
      ...
      await this.setCapability('meter_target_month_to_date', onTarget).catch(this.error);
    }
  }
  ...
  async updateAvgMoney() {
    ...
    if (meterThisMonth) await this.setCapability('meter_money_this_month_avg', moneyThisMonth / meterThisMonth).catch(this.error);

    const moneyThisYear = this.meterMoney.year;
    const meterThisYear = await this.getCapabilityValue(this.ds.cmap.this_year);
    if (meterThisYear) await this.setCapability('meter_money_this_year_avg', moneyThisYear / meterThisYear).catch(this.error);
  }
  ...
  async updateMeasureMinMax({ ...reading }, { ...periods }) {
    ...
    await this.setCapability(this.ds.cmap.measure_source, measureValue).catch(this.error);
