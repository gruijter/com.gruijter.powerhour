...
class MyDevice extends Homey.Device {
  ...
  async fetchPrices() {
    try {
      ...
      if (this.settings.forecastEnable && this.dapForecast && newMarketPrices && newMarketPrices[0]) {
        ...
        let forecast = await this.dapForecast.getPrices({ forecast: true }).catch((err) => this.log(err));
  ...
  async checkPricesValidity(newMarketPrices, periods) {
    ...
    if (oldPrices && oldPrices[0] && oldPrices[0].time) {
      if (newMarketPrices.slice(-1)[0].time < oldPrices.slice(-1)[0].time) throw Error('Fetched prices are older then the stored prices');
    }
    return true;
  }
