...
class MyDevice extends Homey.Device {
  ...
  async restartDevice(delay) {
    ...
    await setTimeoutPromise(dly).then(() => this.onInit().catch((err) => this.error(err)));
  }
  ...
  async createPricesJSON(period) {
    ...
    if (!prices) throw Error(this.homey.__('error_no_prices'));
  ...
  async setVariableMarkup(val) {
    ...
    await this.setSettings({ variableMarkup: val }).catch((err) => this.error(err));
  ...
  async setFixedMarkup(val) {
    ...
    await this.setSettings({ fixedMarkup: val }).catch((err) => this.error(err));
    this.restartDevice(1000).catch((error) => this.error(error));
  }
  ...
  async setFixedMarkupTOD(val) {
    ...
    if (todObject === null) await this.setSettings({ fixedMarkupTOD: '' }).catch((err) => this.error(err));
    else await this.setSettings({ fixedMarkupTOD: val }).catch((err) => this.error(err));
    this.restartDevice(1000).catch((error) => this.error(error));
  }
  ...
  async setFixedMarkupWeekend(val) {
    ...
    await this.setSettings({ fixedMarkupWeekend: val }).catch((err) => this.error(err));
    this.restartDevice(1000).catch((error) => this.error(error));
  }
  ...
  get stepsPerHour() {
    return 60 / this.priceInterval;
  }

  getPricesBefore(args) {
    const currentStep = Math.floor(this.state.Q0 / (this.priceInterval / 15));
    ...
    let offset = 0;
    if (startStep < 0) {
      pricesPartYesterday = this.state.pricesYesterday.slice(startStep);
      offset = pricesPartYesterday.length;
      startStep = 0;
    }
    let pricesPartTomorrow = [];
    if (endStep > stepsPerDay) pricesPartTomorrow = this.state.pricesTomorrow.slice(0, endStep - stepsPerDay);
    const pricesPartToday = this.state.pricesThisDay.slice(startStep, endStep);
  ...
  async priceIsLowest(args) {
    if (!this.state || !this.state.pricesThisDay) throw Error(this.homey.__('error_no_prices'));
    let minimum = Math.min(...this.state.pricesThisDay);
    if (args.period !== 'this_day') minimum = Math.min(...this.state.pricesNext8h.slice(0, Number(args.period) * this.stepsPerHour));
    return this.state.priceNow <= minimum;
  }

  async priceIsLowestToday(args) {
    if (!this.state || !this.state.pricesThisDay) throw Error(this.homey.__('error_no_prices'));
    const lowestNPrices = [...this.state.pricesThisDay].sort((a, b) => a - b).slice(0, args.number * this.stepsPerHour);
  ...
  async priceIsLowestNextHours(args) {
    if (!this.state || !this.state.pricesNextHours) throw Error(this.homey.__('error_no_prices'));
    const period = args.period ? args.period : 99;
    const comingXhours = [...this.state.pricesNextHours].slice(0, period * this.stepsPerHour);
  ...
  async priceIsHighest(args) {
    ...
    if (args.period !== 'this_day') maximum = Math.max(...this.state.pricesNext8h.slice(0, Number(args.period) * this.stepsPerHour));
    return this.state.priceNow >= maximum;
  }

  async priceIsHighestToday(args) {
    if (!this.state || !this.state.pricesThisDay) throw Error(this.homey.__('error_no_prices'));
    const highestNPrices = [...this.state.pricesThisDay].sort((a, b) => a - b).reverse().slice(0, args.number * this.stepsPerHour);
    return this.state.priceNow >= Math.min(...highestNPrices);
  }

  async priceIsHighestNextHours(args) {
    if (!this.state || !this.state.pricesNextHours) throw Error(this.homey.__('error_no_prices'));
    const comingXhours = [...this.state.pricesNextHours].slice(0, args.period * this.stepsPerHour);
  ...
  async setState() {
    ...
    const hourNext8hLowest = (H0 + Math.floor(statsNext8h.minIndex / this.stepsPerHour)) % 24;
    const hourNext8hHighest = (H0 + Math.floor(statsNext8h.maxIndex / this.stepsPerHour)) % 24;
    ...
    const state = {
      ...
      hourThisDayLowest: Math.floor(statsThisDay.minIndex / this.stepsPerHour),
      priceThisDayHighest: statsThisDay.max,
      hourThisDayHighest: Math.floor(statsThisDay.maxIndex / this.stepsPerHour),
      ...
      hourNextDayLowest: statsNextDay.minIndex === null ? null : Math.floor(statsNextDay.minIndex / this.stepsPerHour),
      priceNextDayHighest: statsNextDay.max,
      hourNextDayHighest: statsNextDay.maxIndex === null ? null : Math.floor(statsNextDay.maxIndex / this.stepsPerHour),
    };
    this.state = state;
  }
    ...
    try {
      ...
      if (!options || !options.noTriggers) {
        ...
        if (this.state.pricesNextHours && this.state.pricesNextHours[0]) {
          await this.newPricesReceived(this.state.pricesNextHours, 'next_hours').catch((err) => this.error(err));
        }
        // trigger new prices received right after midnight
        if (this.state.H0 === 0) {
          if (this.state.pricesThisDay && this.state.pricesThisDay[0]) {
            await this.newPricesReceived(this.state.pricesThisDay, 'this_day').catch((err) => this.error(err));
          }
          if (this.state.pricesTomorrow && this.state.pricesTomorrow[0]) {
            await this.newPricesReceived(this.state.pricesTomorrow, 'tomorrow').catch((err) => this.error(err));
          }
        }
