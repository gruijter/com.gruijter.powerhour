...
const getStrategy = ({
  ...
  if (yieldFactor > oldYield) {
    ...
    // Worse than model (Clouds, Shadow, Curtailment, or Over-estimation)
    // On very clear days (high radiation), if power is lower than expected,
    // it's likely model over-estimation, so we should learn down faster.
    alpha = forecastRadiation > 800
      ? 0.2 // Learn down faster to correct over-estimation
      : 0.05; // Keep slow learn down for clouds/shadows
  }
...
const processHistoricData = ({
  ...
  // currentYieldFactors, resolution, and previousAccumulators are no longer used
}) => {
  ...
  const slotBests = new Array(96).fill(0);
  ...
  for (const entry of powerEntries) {
    ...
    // Only consider "good quality" sunny day samples for building the ideal model
    if (radiation > 200) {
      ...
      if (yieldFactor > 0.05 && yieldFactor < 500.0) {
        ...
        // If we find a better yield factor for this slot, record it
        if (yieldFactor > slotBests[slotIndex]) {
          slotBests[slotIndex] = yieldFactor;
        }
        processedSamples++;
      }
    }
  ...
  // Smooth the curve to remove steps and minor outliers (2 passes of weighted moving average)
  // Using a weighted average (2 passes) to better preserve sharp features like shadow dips.
  ...
  return {
    ...
    log: `Retrained model from best-of-day samples. Found ideal yield for ${updatedSlots} slots from ${processedSamples} data points.`,
  };
};
