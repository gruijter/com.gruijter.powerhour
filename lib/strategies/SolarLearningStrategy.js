...
const getStrategy = ({
  ...
  // Dynamic learning rate: higher radiation = higher confidence/relevance. Base 0.05, max 0.2.
  const alpha = Math.min(0.2, Math.max(0.05, 0.06 * (forecastRadiation / 1000)));
  ...
  for (const entry of powerEntries) {
    ...
    if (radiation && radiation > 50) {
      ...
      if (yieldFactor > 0.05 && yieldFactor < 100.0) {
        ...
        const weight = (0.92 ** daysOld) * (radiation / 4000);
...
const calculateForecast = ({
  ...
  const startOfDayTime = startOfDayLocal.getTime() - offset;

  for (let i = 0; i < 96; i++) {
    const slotTime = startOfDayTime + (i * 15 * 60 * 1000);
...
const getSunBounds = (dateObj, forecastData, timezone = 'UTC') => {
  // Calculate offset between UTC and Local Time for the given date
  const now = new Date(dateObj);
  const nowLocal = new Date(now.toLocaleString('en-US', { timeZone: timezone }));
  const offset = nowLocal.getTime() - now.getTime();

  // Calculate start of day in Local Time, then convert back to UTC timestamp
  const startOfDayLocal = new Date(nowLocal);
  startOfDayLocal.setHours(0, 0, 0, 0);
  const startOfDay = new Date(startOfDayLocal.getTime() - offset);
