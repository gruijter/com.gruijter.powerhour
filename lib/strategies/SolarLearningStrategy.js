...
const processHistoricData = ({
  ...
  slotSamples.forEach((samples, idx) => {
    ...
    logger(`[processHistoricData] Slot ${idx} (MaxRad: ${maxPossible.toFixed(0)}W/m2): ${samples.length} samples. Top 25% count=${keepCount}. Best(Median)=${best.toFixed(3)}. TopYields=[${bestYields.map((y) => y.toFixed(2)).join(',')}]`);
    ...
    if (best > slotBests[idx]) {
      ...
      if (best <= limit) {
        logger(`[processHistoricData] Slot ${idx} UPDATED: ${slotBests[idx].toFixed(3)} -> ${best.toFixed(3)} (Limit was ${limit.toFixed(3)})`);
        slotBests[idx] = best;
      } else {
