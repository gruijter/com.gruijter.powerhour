...
const getStrategy = ({
  ...
  // Asymmetric learning:
  // Up: Conservative (0.05) to capture clear sky peaks without over-reacting to momentary cloud-breaks.
  // Down: Very slow (0.01) to adapt to seasonal changes (shading) while resisting transient clouds.
  let alpha = 0;
  let targetYield = yieldFactor;

  if (yieldFactor > oldYield) {
    alpha = 0.05;
    // Cap instantaneous spikes to max +20% above existing slot model
    if (oldYield > 0 && yieldFactor > oldYield * 1.20) {
      targetYield = oldYield * 1.20;
    }
  } else {
    alpha = 0.01;
  }

  // Reject drops > 50% as transient clouds (structural shading is handled by the nightly batch process).
  if (oldYield > 0 && yieldFactor < oldYield) {
    const relativeDrop = (oldYield - yieldFactor) / oldYield;
    if (relativeDrop > 0.50) {
      result.log = `Ignored outlier yield (Cloud Drop ${(relativeDrop * 100).toFixed(0)}%): `
        + `Model=${oldYield.toFixed(2)}, Inst=${yieldFactor.toFixed(2)} (P=${power}, R=${forecastRadiation.toFixed(0)})`;
      return result;
    }
  }
  ...
  if (oldYield === 0) {
    ...
    newYield = (alpha * targetYield) + ((1 - alpha) * oldYield);
  }
...
const processHistoricData = ({
  ...
  // Clear-sky weighting via power-curve roughness was tried and fully reverted
  // (2026-08-14) - see the note near the top of this file for what was tried and
  // why. No trace of it left in this function; ramp-region and normal-region
  // sample selection below are exactly as before that attempt.
  slotSamples.forEach((samples, idx) => {
    ...
    // Evaluate whether this 2-hour window in high-res (24h) data was cloudy.
    ...
    // enough (well past the low-sun ramp) for it to be meaningful.
    let isCloudy = false;
    ...
    if (slotBests[idx] === null) {
      ...
      if (resolution === 'high' && isCloudy) {
        logger(`[processHistoricData] Slot ${idx} SKIP UP: Cloudy day in 2h time block ignored. YF would have become ${best.toFixed(3)}.`);
      } else if (best <= limit) {
