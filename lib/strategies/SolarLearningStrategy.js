...
const detectCurtailment = ({
  ...
  const minExpected = 250; // W - Minimum expected power to check for curtailment (avoids dawn/dusk noise)
  const minExpectedEnd = 100; // W - Lower threshold to end curtailment (hysteresis)
  const offThreshold = 20; // W - Power below this is considered 'off'
  const wasRunningThreshold = Math.max(50, expectedPower * 0.05); // W - Require at least 50W to avoid inverter wake-up spikes
  ...
  if (expectedPower > minExpected && currentPower < offThreshold && lastPower > wasRunningThreshold) {
    ...
    if (expectedPower < minExpectedEnd) {
...
const processHistoricData = ({
  ...
  slotSamples.forEach((samples, idx) => {
    ...
    // DEEP DEBUG: Log ruwe gesorteerde data voor de probleem-uren in de middag (bijv. slot 56 t/m 68 = 14:00 - 17:00)
    if (idx >= 56 && idx <= 68) {
      logger(`[DEBUG] Slot ${idx} heeft ${samples.length} valid samples (gesorteerd op power):`);
      samples.forEach((s, i) => logger(`  -> Sample ${i}: P=${s.power.toFixed(0)}W, R=${s.radiation.toFixed(0)}W/m², YF=${s.yieldFactor.toFixed(3)}`));
    }
    ...
    if (idx >= 56 && idx <= 68) {
      logger(`[DEBUG] Slot ${idx} Top 25% count = ${keepCount}, Gekozen Median YF = ${best.toFixed(3)}`);
    }
    ...
    if (slotBests[idx] === null) {
      ...
      if (resolution === 'high' && isCloudy) {
        if (idx >= 56 && idx <= 68) {
          logger(`[processHistoricData] Slot ${idx} SKIP DOWN: Bewolkte dag in 2u tijdblok genegeerd. YF zou ${best.toFixed(3)} worden.`);
        }
      } else {
        ...
        logger(`[processHistoricData] Slot ${idx} ADJUST DOWN: ${slotBests[idx].toFixed(3)} -> ${newBest.toFixed(3)} | Verlaagd door YF=${best.toFixed(3)} (P=${bestSample.power.toFixed(0)}W, R=${bestSample.radiation.toFixed(0)}W/m²) | Samples totaal: ${samples.length}`);
