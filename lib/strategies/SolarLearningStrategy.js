...
const aggregateYieldFactors = ({
  ...
  //
  // BUG FOUND AND FIXED (2026-08-16, winter cloud analysis): the "reasonably-radiated" bar used
  // to be a hard-coded `radiation > 100`. In a genuine multi-week Dutch winter cloud spell,
  // forecastRadiation can stay under 100 W/m2 across the ENTIRE 14-day window (Haurwitz clear-sky
  // GHI at 52N winter solstice noon is only ~216 W/m2 even with zero cloud, and thick cloud
  // typically attenuates to 10-30% of that) - highRadiationSamples then comes back empty and
  // datasetMaxYF silently stays at its initialized 0, i.e. this entire backstop goes inert
  // exactly when low-SNR data makes it most needed. Fix: derive the bar from the best radiation
  // this specific dataset actually saw (`maxRadSeen`), not a fixed absolute value - a threshold
  // that's fully data-relative can never be empty as long as ANY nonzero-radiation sample was
  // admitted at all. (A clear-sky-relative bar, e.g. a fraction of estimateClearSkyGHI, was
  // considered and rejected: theoretical clear-sky can vastly exceed what a heavily overcast
  // window ever actually observes, so it would reproduce the exact same "empty set" failure
  // under a pessimistic enough cloud cover - only an empirically-observed reference is guaranteed
  // achievable.) Floored at 15 (just above the bare `radiation > 10` admission floor used
  // elsewhere in this file, so the "high" tier still means something above raw noise-floor
  // admission) and capped at 100 (preserves today's behavior unchanged whenever the dataset
  // already has normal high-radiation coverage).
  let maxRadSeen = 0;
  combinedSlotSamples.forEach((samples) => {
    samples.forEach((s) => {
      if (s.radiation > maxRadSeen) maxRadSeen = s.radiation;
    });
  });
  const highRadiationThreshold = Math.min(100, Math.max(15, maxRadSeen * 0.5));
  const highRadiationSamples = [];
  combinedSlotSamples.forEach((samples) => {
    samples.forEach((s) => {
      if (s.radiation >= highRadiationThreshold) highRadiationSamples.push(s);
    });
  });
  let datasetMaxYF = 0;
  ...
  const confidenceValid = new Array(96).fill(false); // true only where a real (non-degenerate) score was computed
  ...
  combinedSlotSamples.forEach((samples, idx) => {
    ...
    // capacity, evaluated at THIS sample's own radiation (never makes the cap tighter, Math.max,
    // so it can only stop wrongly clamping legitimate ramp-hour values near sunrise/sunset, not
    // weaken protection against actually impossible ones).
    //
    // BUG FOUND AND FIXED (2026-08-16, real device data - Oost array slot 16/04:00 UTC): with
    // only 2 samples in a slot and both isFallback (their local window was too small/near-noise-
    // floor to score directly - see MIN_RADIATION_FOR_CONFIDENCE), the percentile-blend-by-count
    // logic above still lands on the higher of the two raw ratios (7.35 and 28.07, true=~1) -
    // and unbounded-below-50 radiation on that sample (13 W/m2) then makes physicalCeiling
    // itself ~380, so it "loosens" a cap that was already correctly tight (datasetMaxYF-based
    // limit was 11.8) into uselessness. A denominator floor alone can't fix this: any floor loose
    // enough to still let genuine moderate-radiation sunrise ramps through (needs to stay well
    // under normal daytime radiation) is nowhere near tight enough to bring a low-hundreds
    // ceiling back down to a single-digit one for any real-sized array. The actual problem is
    // trusting `bestSample.radiation` as a physical anchor at all when that sample was never
    // independently verified (isFallback) - so gate the loosening on that instead: only let a
    // physically-anchored sample loosen the cap when it's the kind of sample this file already
    // trusts enough to score a real confidence weight for (see extractSlotSamples). The
    // MIN_RADIATION_FOR_CONFIDENCE floor stays as a second layer: even a verified sub-slot
    // sample can sit just under it (hourly sub-slot radiation is interpolated independently of
    // the raw hour's own confidence gate, see extractSlotSamples' hourly path).
    if (peakPower > 0 && bestSample.radiation > 0 && !bestSample.isFallback) {
      const physicalCeiling = (peakPower * 1.1) / Math.max(MIN_RADIATION_FOR_CONFIDENCE, bestSample.radiation);
    ...
    //
    // BUG FOUND AND FIXED (2026-08-15, visual check against a real device's rendered chart): a
    // hard +-15% in/out cutoff made the score flip discontinuously between adjacent slots
    // whenever a day's per-slot average sat near the boundary and `best` itself shifted by even
    // a fraction of a percent slot-to-slot (composition of contributing samples changes slightly
    // slot to slot, especially across the hourly/high-res data boundary) - producing sharp,
    // narrow dips with no underlying change in data quality, on a signal that (unlike
    // yieldFactors) also received no cross-slot smoothing. Fixed both: replaced the binary cutoff
    // with a continuous Gaussian falloff (full credit at 0 deviation, ~61% credit at exactly one
    // tolerance-width away, smoothly - never - towards 0 beyond that, no cliff), and added the
    // same 3-pass neighbor smoothing used for yieldFactors below (own true-boundary-only rule).
    const CONFIDENCE_AGREEMENT_SIGMA = 0.15; // relative deviation at which agreement credit is ~61%
    ...
    if (totalDayWeight > 0 && best > 0) {
      const weightedAgreementSum = perDay.reduce((sum, d) => {
        const relDev = Math.abs(d.yieldFactor - best) / best;
        const agreement = Math.exp(-0.5 * (relDev / CONFIDENCE_AGREEMENT_SIGMA) ** 2);
        return sum + (d.weight * agreement);
      }, 0);
      trainingConfidence[idx] = (weightedAgreementSum / totalDayWeight) * 100;
      confidenceValid[idx] = true;
    }
  ...
  // Same spatial smoothing applied to the confidence score, using confidenceValid (not
  // newYieldFactors' null-check) as the "real neighbor" mask - a slot with samples but zero
  // verified (non-fallback) ones has a legitimate 0% score and should NOT be blended in as if
  // it were a genuine data point diluting its neighbors.
  for (let pass = 0; pass < 3; pass += 1) {
    const smoothedConf = [...trainingConfidence];
    for (let i = 1; i < 95; i += 1) {
      if (!confidenceValid[i] || !confidenceValid[i - 1] || !confidenceValid[i + 1]) continue;
      smoothedConf[i] = (trainingConfidence[i - 1] * 0.25) + (trainingConfidence[i] * 0.5) + (trainingConfidence[i + 1] * 0.25);
    }
    if (confidenceValid[0] && confidenceValid[1]) {
      smoothedConf[0] = (trainingConfidence[0] * 0.75) + (trainingConfidence[1] * 0.25);
    }
    if (confidenceValid[95] && confidenceValid[94]) {
      smoothedConf[95] = (trainingConfidence[95] * 0.75) + (trainingConfidence[94] * 0.25);
    }
    for (let i = 0; i < 96; i += 1) trainingConfidence[i] = smoothedConf[i];
  }
...
const mergeYields = ({
  historicYields, previousYields, alpha = 0.7, limit = 0, peakPower = 0, trainingConfidence = [],
}) => {
  ...
  // radiation this module still trusts as daylight - only ever rejects an implausible value,
  // never a legitimate ramp-hour one.
  //
  // BUG FOUND AND FIXED (2026-08-16): that floor used to be 10 W/m2, the bare "is this daylight
  // at all" admission threshold used elsewhere in this file - but for a 1kW array that's a
  // ceiling of 110, which lets through 100x the true yield factor and is no real backstop at
  // all. This function has no per-sample detail to reason about (unlike aggregateYieldFactors,
  // which can gate on isFallback - see its own physicalCeiling fix), so the floor is the only
  // lever available; raise it to MIN_RADIATION_FOR_CONFIDENCE (50), the same "do we actually
  // trust a reading at this radiation" bar already used throughout this file, rather than the
  // much looser bare-admission one - keeps this last-resort backstop meaningfully tighter while
  // still generous enough to never clamp a genuine moderate-radiation ramp-hour value.
  const physicalCeiling = peakPower > 0 ? (peakPower * 1.1) / MIN_RADIATION_FOR_CONFIDENCE : 0;
  ...
  for (let i = 0; i < 96; i += 1) {
    ...
    if (historic !== null && historic !== undefined && previous > 0) {
      ...
      //
      // BUG FOUND AND FIXED (2026-08-16, real device data - Zuid/West arrays, slot 19/04:45
      // UTC): a flat alpha=0.7 trusts tonight's fresh `historic` value the same amount whether
      // it's backed by 14 corroborating days or by a single never-independently-verified
      // (isFallback) sample - real data shows single-sample slots can be genuinely extreme
      // (yf=0.150 on Zuid, 0.179 on West, both from one dawn-lag 04:55 reading where measured
      // power hadn't caught up to the modeled 54 W/m2 radiation yet) and a flat 70% weight lets
      // one such night override most of an otherwise well-established slot. The confidence-
      // weighted "Model Fit" score already computed in aggregateYieldFactors() answers exactly
      // "how corroborated was tonight's value for this slot" - reuse it here (0% confidence -
      // e.g. this exact slot: verified.length===0, see aggregateYieldFactors - fully defers to
      // the established previous value; 100% confidence keeps the normal alpha) rather than
      // inventing a second, disconnected notion of trust. Optional/defaults to unscaled alpha
      // when the caller doesn't have a confidence array (e.g. older callers, tests).
      const conf = trainingConfidence[i];
      const effectiveAlpha = conf === undefined ? alpha : alpha * (conf / 100);
      newYield = (historic * effectiveAlpha) + (previous * (1 - effectiveAlpha));
