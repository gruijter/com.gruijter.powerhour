...
const processHistoricData = ({
  ...
  const slotAccumulators = new Array(96).fill(null).map(() => ({ sum: 0, count: 0 }));
  ...
  for (const entry of powerEntries) {
    ...
    const radiation = weatherMap.get(hourTime.getTime());

    // Only train if we have significant radiation
    if (radiation && radiation > 50) {
      const yieldFactor = power / radiation;

      // Sanity check for yield factor (e.g. 0.05 to 5.0)
      if (yieldFactor > 0.05 && yieldFactor < 5.0) {
        if (resolution === 'hourly') {
          ...
          for (let i = 0; i < 4; i++) {
            ...
            slotAccumulators[slotIndex].sum += yieldFactor;
  ...
  for (let i = 0; i < 96; i++) {
    ...
    if (acc.count >= 5) { // Minimum 5 samples per slot to be statistically relevant
      const avgYield = acc.sum / acc.count;
