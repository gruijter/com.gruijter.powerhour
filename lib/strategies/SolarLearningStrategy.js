...
const getStrategy = ({
  ...
  let alpha = Math.min(0.2, Math.max(0.05, 0.06 * (forecastRadiation / 1000)));

  if (yieldFactor > oldYield) {
    // Performance is better than model: Likely sunny/clear. Learn fast to reach potential.
    alpha = Math.max(alpha, 0.3);
  } else {
    // Performance is worse than model: Likely clouds/timing mismatch. Learn slow to avoid noise.
...
const processHistoricData = ({
  ...
  previousAccumulators = null,
}) => {
  const newYieldFactors = [...currentYieldFactors];
  let effectiveResolution = resolution;
  let resolutionDowngraded = false;

  // Auto-detect resolution if 'high' is requested but data is sparse (e.g. hourly Insights)
  if (resolution === 'high' && powerEntries.length > 10) {
    const startT = new Date(powerEntries[0].t).getTime();
    const endT = new Date(powerEntries[powerEntries.length - 1].t).getTime();
    const hours = (endT - startT) / 3600000;
    if (hours > 1 && (powerEntries.length / hours) < 2.5) {
      effectiveResolution = 'hourly';
      resolutionDowngraded = true;
    }
  }

  const slotAccumulators = previousAccumulators
    ? previousAccumulators.map((a) => ({ ...a }))
    : new Array(96).fill(null).map(() => ({ weightedSum: 0, totalWeight: 0, count: 0 }));
  ...
  for (const entry of powerEntries) {
    ...
    if (radiation && radiation > 50) {
      ...
      if (yieldFactor > 0.05 && yieldFactor < 100.0) {
        // Asymmetric weighting: Trust "better" days (sunny) much more than "worse" days (cloudy)
        // This helps the model converge to the 'Clear Sky' potential instead of the cloudy average.
        const entrySlot = (timestamp.getUTCHours() * 4) + Math.floor(timestamp.getUTCMinutes() / 15);
        const currentModelYield = currentYieldFactors[entrySlot] || 0;
        let asymmetry = 1.0;

        if (yieldFactor > currentModelYield) {
          asymmetry = 5.0; // Boost sunny days significantly
        } else {
          asymmetry = 0.2; // Suppress cloudy days
        }

        // Weight recent data higher (exponential decay ~8% per day) AND weight higher radiation higher
        const daysOld = Math.max(0, (now - timestamp.getTime()) / oneDay);
        const weight = (0.92 ** daysOld) * (radiation / 4000) * asymmetry;

        if (effectiveResolution === 'hourly') {
          // Apply to all 4 slots in this hour since historic data is hourly
          const startSlot = timestamp.getUTCHours() * 4;
          for (let i = 0; i < 4; i++) {
  ...
  for (let i = 0; i < 96; i++) {
    ...
    if (acc.count >= 2) { // Minimum 2 samples per slot to be statistically relevant
  ...
  return {
    ...
    log: `Retrained model. Updated ${updatedSlots} slots based on historic data.`
      + `${resolutionDowngraded ? ' (Resolution downgraded to hourly due to sparse data)' : ''}`
      + `${updatedSlots < 10 ? ' (Note: Sparse updates expected due to Homey data compression >7 days)' : ''}`,
  };
};
