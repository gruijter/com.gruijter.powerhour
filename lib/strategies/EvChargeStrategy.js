...
const getStrategy = ({
  ...
  let depDate;
  if (tripOverrideDate && tripOverrideTime) {
    const [depH, depM] = tripOverrideTime.split(':').map(Number);
    depDate = new Date(`${tripOverrideDate}T${String(depH).padStart(2, '0')}:${String(depM).padStart(2, '0')}:00`);
    if (Number.isNaN(depDate.getTime())) {
      depDate = new Date(nowLocal);
      depDate.setHours(depH, depM, 0, 0);
      if (depDate <= nowLocal) depDate.setDate(depDate.getDate() + 1);
    }
  } else {
    const [depH, depM] = (tripOverrideTime || departureTime).split(':').map(Number);
    depDate = new Date(nowLocal);
    depDate.setHours(depH, depM, 0, 0);
    if (depDate <= nowLocal) {
  ...
  const hoursUntilDeparture = Math.max(0.1, (depDate - nowLocal) / 3600000);
