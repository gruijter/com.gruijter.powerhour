...
const getStrategy = ({
  ...
  if (effectiveDep === 'indefinite') {
    ...
    hoursUntilDeparture = Math.max(1, (calcPrices.length * priceInterval) / 60);
  ...
  const maxIntervals = Math.min(calcPrices.length, intervalsUntilDeparture);
  const validPrices = calcPrices.slice(0, maxIntervals).map((price, idx) => ({ price, idx })).sort((a, b) => a.price - b.price);
