...
// Charger power (W) above which the charger is considered "active" for session-boundary
// detection - both here (Insights backfill) and in device.js's live power-gap connection-state
// fallback for chargers with no connection-state capability of their own.
const ACTIVE_POWER_THRESHOLD = 20;
...
const bootstrapFromHistory = (powerEntries, socEntries, timezone, batCapacity) => {
  ...
  const sortedSoc = (socEntries && socEntries.length > 0)
    ? [...socEntries].sort((a, b) => a.t - b.t)
    : null;

  const socLookup = (ts) => {
    if (!sortedSoc || sortedSoc.length === 0) return null;
    let low = 0;
    let high = sortedSoc.length - 1;
    while (low <= high) {
      const mid = Math.floor((low + high) / 2);
      if (sortedSoc[mid].t < ts) {
        low = mid + 1;
      } else {
        high = mid - 1;
      }
    ...
    for (const idx of [high, low]) {
      if (idx >= 0 && idx < sortedSoc.length) {
        const diff = Math.abs(sortedSoc[idx].t - ts);
        if (diff < minDiff) {
          minDiff = diff;
          best = sortedSoc[idx];
        }
    ...
    if (best && minDiff < 15 * 60 * 1000) return best.v;
    return null;
  };
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    const wasActive = prev.v > ACTIVE_POWER_THRESHOLD;
    const isActive = curr.v > ACTIVE_POWER_THRESHOLD;
