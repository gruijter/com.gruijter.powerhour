...
const getStrategy = ({
  ...
  // find peaks and troughs (top/bottom third of prices)
  ...
  const peakPrices = prcs
    ...
    .slice(0, 10);
  if (peakPrices[0] !== prices[0]) return 0; // return Hold if first hour is not a peak/trough candidate
  ...
  const stateAfter = (stateBefore) => {
    ...
    // evaluate each candidate strategy (hold, and sell/buy if applicable)
    ...
    if (price === undefined) {
      ...
      finished.socValue = finished.soc * batCapPerc * avgPrice;
