...
const getStrategy = ({
  ...
  const maxSteps = 96; // Cap total steps to ~96 to keep memory usage safe
  ...
  const maxChgKW = Math.max(...chargeSpeeds.map((s) => s.power)) / 1000;
  const maxDisChgKW = Math.max(...dischargeSpeeds.map((s) => s.power)) / 1000;
  ...
  [...calcPrices].forEach((price, t) => {
    ...
    // 4. Grid balance constraint: G_in - G_out - Charge_Grid + Discharge_Grid = eHome
    const eHome = (homePowerUsage / 1000) * durationHours;
    const maxImport = Math.max(0, eHome) + (maxChgKW * durationHours);
    const maxExport = Math.max(0, -eHome) + (maxDisChgKW * durationHours);
    model.constraints[`grid_bal_${t}`] = { equal: eHome };

    model.constraints[`gin_max_${t}`] = { max: maxImport };
    model.variables[`gin_T${t}`] = {
      totalCost: price,
      [`grid_bal_${t}`]: 1,
      [`gin_max_${t}`]: 1,
    };

    model.constraints[`gout_max_${t}`] = { max: maxExport };
    model.variables[`gout_T${t}`] = {
      totalCost: -feedbackPrice,
      [`grid_bal_${t}`]: -1,
      [`gout_max_${t}`]: 1,
    };
    ...
    [...chargeSpeeds].forEach((speed, idx) => {
      ...
      const energyBat = powerKW * durationHours * speed.eff; // Energy added to battery
      const energyGrid = powerKW * durationHours; // Energy pulled from grid
      const cost = powerKW * durationHours * fc; // Wear and tear cost only

      model.variables[varName] = {
        totalCost: cost,
        [`time_${t}`]: 1,
        [`bal_${t}`]: -energyBat, // Moves to RHS as +energy
        [`grid_bal_${t}`]: -energyGrid,
      };
    });
    ...
    [...dischargeSpeeds].forEach((speed, idx) => {
      ...
      const powerKW = speed.power / 1000;
      const energyBat = (powerKW * durationHours) / speed.eff; // Energy removed from battery
      const energyGrid = powerKW * durationHours; // Energy pushed to grid
      const cost = powerKW * durationHours * fc; // Wear and tear cost only
