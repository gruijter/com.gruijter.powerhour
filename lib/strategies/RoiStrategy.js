...
const getStrategy = ({
  ...
  [...calcPrices].forEach((price, t) => {
    ...
    [...chargeSpeeds].forEach((speed, idx) => {
      ...
      const energyBat = pKW * durationHours * speed.eff; // Energy added to battery
      const cost = (chargeCostPerHour + pKW * fc) * durationHours;
    ...
    [...dischargeSpeeds].forEach((speed, idx) => {
      ...
      const energyBat = (pKW * durationHours) / speed.eff; // Energy removed from battery
      const cost = -(dischargeRevPerHour - pKW * fc) * durationHours; // Revenue is negative cost
