...
const getLocalTimeDetails = (timestamp, timezone) => {
  const localDate = TimeHelpers.toLocalDate(timestamp, timezone);
...
const calculateForecast = ({
  ...
  let forecastH0Kwh = 0;
  const startOfHour = TimeHelpers.startOfLocalBlock(now, 60, timezone);
  for (let i = 0; i < 4; i += 1) {
...
const summarizeForecastErrors = (history, timezone = 'UTC') => {
  ...
  const hours = list.map((e) => TimeHelpers.toLocalDate(e.time, timezone).getHours());
  const blocks = {};
