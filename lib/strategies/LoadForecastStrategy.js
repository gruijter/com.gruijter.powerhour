...
// One Intl formatter per timezone, reused: toLocaleString() per call is far slower, and this runs
// for every 15-minute slot of every forecast series.
...
const getLocalTimeDetails = (timestamp, timezone) => {
  ...
  let fmt = localFormatters.get(timezone);
  if (!fmt) {
    fmt = new Intl.DateTimeFormat('en-US', {
      timeZone: timezone, weekday: 'short', hour: 'numeric', minute: 'numeric', hourCycle: 'h23',
    });
    localFormatters.set(timezone, fmt);
  }
  const parts = {};
  fmt.formatToParts(date).forEach((p) => {
    parts[p.type] = p.value;
  });
  const dayOfWeek = WEEKDAYS[parts.weekday]; // 0 = Sunday, 1 = Monday, ..., 6 = Saturday
  const slotIndex = (Number(parts.hour) * 4) + Math.floor(Number(parts.minute) / 15);
  ...
  if (currentSlotIndex !== result.bucket.index) {
    ...
    if (valid) {
      result.finishedBucket = {
        ...
        durationMs: currentTimestamp - result.bucket.startTime,
        log,
      };
    }
...
const processHistoricData = ({
  ...
  timezone = 'UTC',
  logger = () => { },
  // Plausibility ceiling for a home-load training sample. Defaults to the historical flat
  // 30000 W; the grid device passes its own connection-derived ceiling so the bound scales with
  // the actual grid connection instead of quietly dropping real samples above 30 kW.
  maxPowerW = 30000,
}) => {
  ...
  powerEntries.forEach((entry) => {
    ...
    if (typeof power !== 'number' || Number.isNaN(power) || power < 0 || power > maxPowerW) return;

    // NOTE (2026-08-14): powerEntries here is Homey 'last14Days' hourly data, where each
    // entry's value is the AVERAGE over the hour starting at its timestamp, not a snapshot -
    // same confirmed fact as SolarLearningStrategy.js. A midpoint-anchoring fix analogous to
    // solar's was tried and reverted: validated against this device's own real remote data
    // (14-day train / held-out day's real 5-minute ground truth), it made MAE slightly WORSE
    // (307W -> 333W), because home load is a sum of abrupt, event-driven on/off transitions
    // (appliances, battery inverter, EV charger) rather than a smooth, near-linear-within-
    // the-hour curve like solar irradiance - the "average represents the midpoint" reasoning
    // only pays off for the latter. Left as-is (no shift) pending a broader/multi-day
    // re-test if this is revisited.
  ...
  if (globalNightValues.length > 0) {
    ...
    logger(`[processHistoricData] Standby floor selected = ${nightFloor}W (p10, validSlots=${globalNightValues.length})`);
  }
...
const calculateForecast = ({
  ...
  let totalTodayKwh = 0;
  // Real local midnights: a DST day has 23 or 25 hours. +26h always lands in the next local day.
  const nextMidnight = (ms) => TimeHelpers.getLocalMidnightUTC(new Date(ms + 26 * 60 * 60 * 1000), timezone).getTime();
  const startOfTodayTime = TimeHelpers.getLocalMidnightUTC(now, timezone).getTime();
  const endOfTodayTime = nextMidnight(startOfTodayTime);
  ...
  const endOfTomorrowTime = nextMidnight(startOfTomorrowTime);
...
// Forecast home load (W) per 15-minute slot from startMs (inclusive) to endMs (exclusive).
const getLoadSeries = (weeklyProfile, startMs, endMs, timezone = 'UTC') => {
  const profile = weeklyProfile || initializeProfile();
  const values = [];
  for (let t = startMs; t < endMs; t += 15 * 60 * 1000) {
    const { dayOfWeek, slotIndex } = getLocalTimeDetails(t, timezone);
    values.push(profile[dayOfWeek][slotIndex] || 0);
  }
  return values;
};

// Forecast accuracy. One entry per closed 15-minute slot: the forecast that was in effect for
// that slot (read before the slot's own learning update) against what actually happened.
const FORECAST_ERROR_WINDOW = 7 * 96; // 7 days of 15-minute slots

const recordForecastError = (history, { time, predicted, actual }, maxEntries = FORECAST_ERROR_WINDOW) => {
  const list = Array.isArray(history) ? history.slice(-(maxEntries - 1)) : [];
  if ([time, predicted, actual].every((v) => typeof v === 'number' && Number.isFinite(v))) {
    list.push({ time, predicted: Math.round(predicted), actual: Math.round(actual) });
  }
  return list;
};

// Time-of-day blocks for the per-block breakdown, by local start hour.
const ERROR_BLOCKS = [
...
// errors are predicted - actual: positive bias means the forecast runs high.
// maePct is MAE relative to the mean actual load, null when there is no load at all.
const summarizeForecastErrors = (history, timezone = 'UTC') => {
  const stats = (entries) => {
    ...
    return {
      samples: n,
      maeW: Math.round(mae),
      biasW: Math.round(sum / n),
      maePct: actualSum > 0 ? Math.round((mae / (actualSum / n)) * 100) : null,
    };
  };
  const list = Array.isArray(history) ? history : [];
  // One reused formatter: toLocaleString per entry is slow, and this runs on flow triggers.
  const hourFormat = new Intl.DateTimeFormat('en-US', { timeZone: timezone, hour: 'numeric', hourCycle: 'h23' });
  const hours = list.map((e) => Number(hourFormat.format(new Date(e.time))));
  const blocks = {};
  ERROR_BLOCKS.forEach(({ name, from, to }) => {
    blocks[name] = stats(list.filter((_, i) => hours[i] >= from && hours[i] < to));
  });
  return { ...stats(list), blocks };
};
