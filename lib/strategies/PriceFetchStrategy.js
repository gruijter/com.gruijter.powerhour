...
class PriceFetchStrategy {
   ...
   */
  static canImprove(storedPrices, providerName, periods, isNordpoolZone, priceInterval) {
    ...
    const rank = this.getProviderRank(providerName, isNordpoolZone);
    ...
    storedPrices.forEach((price) => {
      ...
      covered += 1;
      if (this.getProviderRank(price.provider, isNordpoolZone) > rank) improvable = true;
    });
  ...
  static async fetchPrices(providers, periods, resolution, validator, logger, options = {}) {
    const {
      storedPrices = [], isNordpoolZone = false, priceInterval = 60, callState = new Map(),
    } = options;
    ...
    for (const provider of providers) {
      ...
      if (!this.canImprove(storedPrices, providerName, periods, isNordpoolZone, priceInterval)) {
