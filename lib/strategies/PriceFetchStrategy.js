...
class PriceFetchStrategy {
  ...
  static async fetchPrices(providers, periods, resolution, validator, logger, primaryOnly = false, isNordpoolZone = false) {
    ...
    if (logger) logger(`Starting fetch loop with providers: ${providersToUse.map((p) => p.name).join(', ')}`);
    ...
    for (const provider of providersToUse) {
      try {
        ...
        if (hasTomorrow) {
          if (logger) logger(`Got tomorrow's prices from ${provider.constructor.name}`);
        ...
        if (logger) logger(`${provider.constructor.name} has no prices for tomorrow yet. Trying next...`);
      } catch (err) {
        if (err.message === 'Session changed') throw err; // Abort immediately
        if (err.message !== 'Fetched prices are older then the stored prices') {
          if (logger) logger(`Error fetching from ${provider.constructor.name}: ${err.message}`);
        } else if (logger) {
          logger(`${provider.constructor.name} returned prices older than stored prices. Skipping...`);
        }
      }
    }
