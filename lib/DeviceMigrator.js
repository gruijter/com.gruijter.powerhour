...
module.exports = {
  ...
  // Chart image capabilities (todayChargeChart, gridWeekly, etc.) are added dynamically via
  // device.setCameraImage() the first time each one is generated, guarded by an in-memory
  // `if (!this.xImage)` check - they're never listed in a driver's deviceCapabilities array, so
  // migrateCapabilities() above (which only reorders array-declared capabilities) can't fix their
  // order. Homey keeps a capability's position from when it was first added, so the only way to
  // change the display order for an already-paired device is to remove it and let it be re-added.
  // Gated by a store flag so this only runs once per device, not on every onInit()/restartDevice().
  async migrateImageCapabilityOrder(device, capImageMap, storeKey) {
    if (await device.getStoreValue(storeKey)) return;
    const entries = Object.entries(capImageMap);
    for (const [cap] of entries) {
      if (device.hasCapability(cap)) {
        device.log(`Removing chart capability ${cap} to fix its display order`);
        await device.removeCapability(cap).catch((err) => device.error(err));
      }
      // Also clear the in-memory Image reference (restartDevice() re-runs onInit() on the same
      // device instance) so the next chart update recreates the capability instead of skipping
      // it via the `if (!this.xImage)` guard and writing to a stream for a capability that no
      // longer exists.
      device[capImageMap[cap]] = undefined;
    }
    await device.setStoreValue(storeKey, true).catch((err) => device.error(err));
  },
