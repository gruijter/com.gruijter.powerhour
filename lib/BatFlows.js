...
class BatFlows {
  ...
  async price_batt_best_trade(args) {
    ...
    if (!this.device.pricesNextHours) throw Error(this.device.homey.__('error_no_prices'));
  ...
  async find_roi_strategy(args) {
    try {
      ...
      if (!this.device.pricesNextHours) throw Error(this.device.homey.__('error_no_prices'));
      if (!this.device.priceInterval) throw Error(this.device.homey.__('error_no_price_interval'));
      ...
      if (this.device.lastStratOptsString === stratOptsString) {
        ...
        return Promise.resolve(this.device.lastStratTokens);
      }
  ...
  async reTriggerNewRoiStrategyFlow(tokens, args) {
    try {
      ...
      const now = new Date();
      const startMinute = now.getMinutes();
      if ((startMinute % this.device.priceInterval) + duration >= (this.device.priceInterval - 5)) return; // do not retrigger if duration is crossing to next period
