...
class ENTSOE extends PriceProvider {
  ...
  async getPrices(options) {
    ...
    const filtered = tsArr.filter((s) => usableResolutions.includes(s.Period.resolution._text));

    let prices = [];
    for (const s of filtered) {
      const period = s.Period;
      const seriesResolution = period.resolution._text;
      let interval = 60;
      if (seriesResolution === 'PT15M') interval = 15;
      if (seriesResolution === 'PT30M') interval = 30;
