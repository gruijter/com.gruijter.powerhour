...
class ENTSOE extends PriceProvider {
  ...
  async getPrices(options) {
    ...
    let fetchResolution = resolution;
    if (resolution === 'PT60M') fetchResolution = 'PT15M';

    // interval is determined later based on actual fetched resolution
    ...
    let filtered = tsArr.filter((s) => s.Period.resolution._text === fetchResolution);

    // Fallback to PT60M if PT15M was preferred (for averaging) but not found
    if (filtered.length === 0 && resolution === 'PT60M' && fetchResolution === 'PT15M') {
      fetchResolution = 'PT60M';
      filtered = tsArr.filter((s) => s.Period.resolution._text === fetchResolution);
    }

    // Fallback to PT15M if PT30M was requested but not found
    if (filtered.length === 0 && resolution === 'PT30M') {
      fetchResolution = 'PT15M';
      filtered = tsArr.filter((s) => s.Period.resolution._text === fetchResolution);
    }

    let interval = 60;
    if (fetchResolution === 'PT15M') interval = 15;
    if (fetchResolution === 'PT30M') interval = 30;
    ...
    // If resolution is PT30M, but we fetched PT15M, average to 30m
    if (opts.resolution === 'PT30M' && fetchResolution === 'PT15M') {
