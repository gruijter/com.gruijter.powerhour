...
class ENTSOE extends PriceProvider {
  ...
  async getPrices(options) {
    ...
    for (const s of filtered) {
      ...
      for (let i = 1; i <= expectedPoints; i += 1) {
        ...
        if (currentPrice !== null) {
          ...
          // skip hours already covered by a finer resolution, and mRID duplicates
          if ((claimed === undefined || claimed >= interval)
            && !prices.some((entry) => entry.time.getTime() === time.getTime())) {
            prices.push({
              time,
              price: currentPrice,
            });
    ...
    // If resolution is PT30M, average prices per half hour and keep only half hourly timestamps
