...
const padMissingHours = (data) => {
  ...
  data.forEach((currentEntry, idx) => {
    ...
    if (idx > 0) {
      ...
      hoursDiff = (currTime - prevTime) / (1000 * 60 * 60);
      while (hoursDiff > 1) {
    ...
    if (hoursDiff > 0) paddedData.push(data[idx]); // skip duplicate/out-of-order timestamps
  });
  return (paddedData);
};
...
class Energyzero extends PriceProvider {
  ...
  async getPrices(options) {
    try {
      ...
      const res = await fetch(url, { method: 'GET', signal: AbortSignal.timeout(this.timeout) });
