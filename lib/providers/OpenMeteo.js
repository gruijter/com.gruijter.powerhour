...
class OpenMeteo {
  ...
  static async fetchForecast(lat, lon) {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=shortwave_radiation_instant&forecast_days=2&timezone=UTC`;
  ...
  static async fetchHistoric(lat, lon, startDate, endDate) {
    ...
    let data;
    try {
      // 1. Try Forecast API (High resolution, recent history)
      ...
      // 2. Fallback to Archive API (Low resolution, long history)
      ...
      const response = await fetch(url);
      if (!response.ok) throw new Error(`Archive API error: ${response.statusText}`);
      data = await response.json();
    }
