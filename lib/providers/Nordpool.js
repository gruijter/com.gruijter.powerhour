...
class Nordpool extends PriceProvider {
  ...
  async getPrices(options) {
    ...
    let resolution = opts.resolution || 'PT60M'; // Default to 60m if not specified
    if (resolution !== 'PT15M' && resolution !== 'PT30M') resolution = 'PT60M';
    const aggregation = resolution === 'PT60M' ? 'Hourly' : 'DeliveryPeriod';
    ...
    // Check if we have 15m data if requested (BEFORE padding)
    if (resolution === 'PT15M') {
