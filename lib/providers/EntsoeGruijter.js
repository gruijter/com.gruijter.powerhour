/*
Copyright 2019 - 2026, Robin de Gruijter (gruijter@hotmail.com)

This file is part of com.gruijter.powerhour.

com.gruijter.powerhour is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.

com.gruijter.powerhour is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
GNU General Public License for more details.

You should have received a copy of the GNU General Public License
along with com.gruijter.powerhour.  If not, see <http://www.gnu.org/licenses/>.
*/

'use strict';

const PriceProvider = require('./PriceProvider');

const defaultHost = 'entsoe-prices.gruijter.org';
const defaultPort = 443;
const defaultTimeout = 30000;

const biddingZones = {
  AL_Albania: '10YAL-KESH-----5',
  AT_Austria: '10YAT-APG------L',
  BE_Belgium: '10YBE----------2',
  BG_Bulgaria: '10YCA-BULGARIA-R',
  CH_Switzerland: '10YCH-SWISSGRIDZ',
  CR_Croatia: '10YHR-HEP------M',
  CZ_Czech_Republic_CEPS: '10YCZ-CEPS-----N',
  DE_Germany_DE_LU: '10Y1001A1001A82H',
  DK_Denmark_1: '10YDK-1--------W',
  DK_Denmark_2: '10YDK-2--------M',
  EE_Estonia: '10Y1001A1001A39I',
  ES_Spain: '10YES-REE------0',
  FI_Finland: '10YFI-1--------U',
  FR_France: '10YFR-RTE------C',
  GR_Greece: '10YGR-HTSO-----Y',
  // '10Y1001A1001A59C' is the SEM (all-island Irish) bidding zone, NOT an Amprion feed.
  // It was declared here as DE_Germany_Amprion until 2026-09-10. ENTSO-E's approved EIC
  // registry gives it EicDisplayName 'SEM' / EicLongName 'Ireland and Northern Ireland',
  // and there is no Y-area bidding-zone code for Amprion at all - Amprion is a TSO inside
  // the DE-LU bidding zone and has no day-ahead price of its own. Confirmed by data: this
  // series is 0/48 identical to DE-LU and 48/48 identical to ENTSO-E's own SEM feed.
  HU_Hungary: '10YHU-MAVIR----U',
  IE_Ireland_SEM: '10Y1001A1001A59C',
  IT_Italy_Center_South: '10Y1001A1001A71M',
  IT_Italy_Centre_North: '10Y1001A1001A70O',
  IT_Italy_North: '10Y1001A1001A73I',
  // IT_Italy_Saco_DC ('10Y1001A1001A893') is deliberately NOT offered: the proxy serves it, but
  // every point comes back as a flat 1700 EUR/MWh - the market price cap, i.e. a congestion
  // sentinel for this virtual DC-link area, not a price anyone consumes at. Entsoe.js lists it
  // for completeness; a user-selectable zone it should not be.
  IT_Italy_Saco_AC: '10Y1001A1001A885',
  IT_Italy_Sardinia: '10Y1001A1001A74G',
  IT_Italy_Sicily: '10Y1001A1001A75E',
  IT_Italy_South: '10Y1001A1001A788',
  LT_Lithuania: '10YLT-1001A0008Q',
  LV_Latvia: '10YLV-1001A00074',
  ME_Montenegro: '10YCS-CG-TSO---S',
  MK_North_Macedonia: '10YMK-MEPSO----8',
  NL_Netherlands: '10YNL----------L',
  NO_Norway_1: '10YNO-1--------2',
  NO_Norway_2: '10YNO-2--------T',
  NO_Norway_2NSL: '50Y0JVU59B4JWQCU',
  NO_Norway_3: '10YNO-3--------J',
  NO_Norway_4: '10YNO-4--------9',
  NO_Norway_5: '10Y1001A1001A48H',
  PL_Poland_PSE: '10YPL-AREA-----S',
  PT_Portugal: '10YPT-REN------W',
  RO_Romania: '10YRO-TEL------P',
  RS_Serbia: '10YCS-SERBIATSOV',
  SE_Sweden_1: '10Y1001A1001A44P',
  SE_Sweden_2: '10Y1001A1001A45N',
  SE_Sweden_3: '10Y1001A1001A46L',
  SE_Sweden_4: '10Y1001A1001A47J',
  SI_Slovenia: '10YSI-ELES-----O',
  SK_Slovakia_SEPS: '10YSK-SEPS-----K',
  UA_Ukraine_IPS: '10Y1001C--000182',
  XK_Kosovo: '10Y1001C--00100H',
};

// Represents a session to the ENTSOE.GRUIJTER.ORG API.
class ENTSOE_GRUIJTER extends PriceProvider {

  constructor(opts) {
    super(opts);
    const options = opts || {};
    this.host = options.host || defaultHost;
    this.port = options.port || defaultPort;
    this.timeout = options.timeout || defaultTimeout;
    this.apiKey = options.apiKey || '';
    this.biddingZone = options.biddingZone;
    this.resolution = options.resolution || 'PT15M'; // 'PT15M', 'PT30M' or 'PT60M'
    this.biddingZones = biddingZones;
    this.lastResponse = undefined;
  }

  getBiddingZones() {
    return this.biddingZones;
  }

  /**
   * EntsoeGruijter Proxy.
   * Based on Cloudflare R2 with 5 minute CDN buffering.
   * Uses a 5-minute spread to avoid overloading the proxy server.
   */
  // Own proxy: cheap to call, so no minimum interval and only a short spread. Revalidation is
  // practically free because the proxy answers a conditional request with 304 and an empty body.
  getRateLimit() {
    return {
      minDelay: 30000,
      maxRandomDelay: 60000,
      minInterval: 0,
      maxBackoff: 60 * 60 * 1000,
      revalidateInterval: 60 * 60 * 1000,
    };
  }

  /**
  * Get the prices
  * @returns {(Promise.[priceInfo])}
  * @property {string} [biddingZone] - e.g. '10YNL----------L'
  * @property {string} [dateStart = today] - date Object or date string, e.g. '2022-02-21T20:36:10.665Z'
  * @property {string} [dateEnd = tomorrow ] - date Object or date string, e.g. '2022-02-21T20:36:10.665Z'
  * @property {string} [resolution] - 'PT15M', 'PT30M' or 'PT60M'
  */
  async getPrices(options) {
    const today = new Date();
    const tomorrow = new Date(today);
    tomorrow.setDate(today.getDate() + 1);

    const opts = options || {};
    const zone = opts.biddingZone || this.biddingZone;
    const start = opts.dateStart ? new Date(opts.dateStart) : today;
    const end = opts.dateEnd ? new Date(opts.dateEnd) : tomorrow;
    let resolution = opts.resolution || this.resolution;

    // On a revalidation we only want to know whether anything changed. The proxy answers a
    // conditional request with 304 and an empty body, which _makeRequest reports as notModified.
    const path = `/${zone}.json?ts=${Date.now()}`;
    const res = await this._makeRequest(path, undefined, opts.revalidateOnly ? this.etag : null);

    if (!res?.data) throw Error('contains no prices data');
    if (resolution === 'PT60M' && res.res === '15m') resolution = 'PT15M';
    if (resolution === 'PT30M' && res.res === '15m') resolution = 'PT15M';
    if (resolution === 'PT30M' && res.res !== '30m' && res.res !== '15m') throw Error('No 30m resolution available');
    if (resolution === 'PT15M' && res.res !== '15m') throw Error('No 15m resolution available');

    let prices = res.data
      .map((item) => ({ ...item, time: new Date(item.time) }))
      .filter((p) => p.time >= start && p.time <= end).sort((a, b) => a.time - b.time)
      .filter((item, pos, ary) => !pos || item.time.getTime() !== ary[pos - 1].time.getTime()); // remove duplicates

    if (prices.length === 0) throw Error('No prices found for requested period');

    // Validate interval consistency
    if (prices.length > 1) {
      let expectedInterval = 60 * 60000;
      if (resolution === 'PT15M') expectedInterval = 15 * 60000;
      else if (resolution === 'PT30M') expectedInterval = 30 * 60000;
      for (let i = 1; i < prices.length; i += 1) {
        const diff = prices[i].time.getTime() - prices[i - 1].time.getTime();
        if (diff !== expectedInterval) throw Error(`Invalid interval detected: ${diff / 60000}m (expected ${expectedInterval / 60000}m)`);
      }
    }

    // Average down when a coarser resolution was requested than what was returned
    if (opts.resolution === 'PT30M') prices = PriceProvider.aggregateTo(prices, 30);
    if (opts.resolution === 'PT60M') prices = PriceProvider.aggregateTo(prices, 60);

    return prices;
  }

  /**
   * @param {string} [etag] - when given, the request is conditional. A 304 response rejects with an
   * error carrying notModified = true, so the caller can tell 'unchanged' from an actual failure.
   */
  async _makeRequest(actionPath, timeout, etag) {
    try {
      const url = `https://${this.host}${actionPath}`;
      const reqTimeout = timeout || this.timeout;
      const options = {
        method: 'GET',
        signal: AbortSignal.timeout(reqTimeout),
      };
      if (etag) options.headers = { 'If-None-Match': etag };

      const result = await fetch(url, options);
      this.lastResponse = result.status;

      if (result.status === 304) {
        const notModified = Error('Prices unchanged since last fetch');
        notModified.notModified = true;
        throw notModified;
      }
      if (result.status === 404) throw Error(`Zone not found: ${actionPath}`);

      // find errors
      const body = await result.text();
      this.lastResponse = body || result.status;
      const contentType = result.headers.get('content-type');
      if (!/\/json/.test(contentType)) {
        throw Error(`Expected JSON but received ${contentType}: ${body.slice(0, 100)}`);
      }
      if (result.status !== 200) {
        this.lastResponse = result.status;
        throw Error(`HTTPS request Failed. Status Code: ${result.status}`);
      }
      this.etag = result.headers.get('etag');
      return JSON.parse(body);
    } catch (error) {
      this.lastResponse = error;
      throw error;
    }
  }
}

module.exports = ENTSOE_GRUIJTER;
