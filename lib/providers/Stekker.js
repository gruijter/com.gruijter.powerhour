...
// MUST stay in sync with biddingZonesMap below: a zone listed here but missing (or commented out)
// there is offered during pairing and then throws 'biddingZone not supported by Stekker' on every
// fetch. 11 such zones were removed on 2026-09-10 after sweeping all 40 against the live service -
// Bulgaria and all nine Italian zones were already commented out of the map, and DE_Germany_DE_LU
// was mapped but answers 422 'Region not supported' (verified on repeated attempts, both
// resolutions, while NL succeeded in the same run).
const biddingZones = {
...
const biddingZonesMap = {
  ...
  // '10Y1001A1001A82H': ['DE-LU-3600', 'DE-LU-900'], // DE_Germany_DE_LU - API answers 422 Region not supported
...
class Stekker extends PriceProvider {
   ...
   */
  getRateLimit() {
    // Lowest ranking source, its prices are replaced by any better provider anyway: never revalidate
  ...
  async getPrices(options) {
    try {
      ...
      // Stekker serves each bidding zone under separate hourly (-3600) and sub-hourly (-900/-1800)
      // regions. Try the region matching the requested resolution first, and fall back to the other
      // one when it returns nothing, so e.g. 60m prices can still be built from 15m data.
      const zoneCodes = biddingZonesMap[zone];
      const subHourlyRegion = zoneCodes.find((c) => c.includes('-900') || c.includes('-1800'));
      const hourlyRegion = zoneCodes.find((c) => c.includes('-3600')) || zoneCodes[0];
      let regions = (resolution === 'PT15M' || resolution === 'PT30M')
        ? [subHourlyRegion, hourlyRegion]
        : [hourlyRegion, subHourlyRegion];
      regions = regions.filter((region, idx) => region && regions.indexOf(region) === idx);
      if (regionOverride) regions = [regionOverride];

      let info = null;
      let regionError;
      for (const region of regions) {
        try {
          const regionInfo = await this._getRegionPrices(region, { startDate, endDate, forecast });
      ...
      if (!info) throw regionError || Error('no price data found');
      ...
      if (resolution === 'PT15M' && info.length > 0) {
        ...
        if (!hasSub30m) {
          // Fallback: Interpolate from 30m or 60m data
      ...
      if (resolution === 'PT30M' && info.length > 0) {
        ...
        const hasSub30m = info.some((p, i) => i > 0 && (p.time - info[i - 1].time) < 1800000);
        if (hasSub30m) info = PriceProvider.aggregateTo(info, 30);
      }

      // Average to 60 minute intervals if requested
      if (resolution === 'PT60M' && info.length > 0) {
        const hasSubHourly = info.some((p, i) => i > 0 && (p.time - info[i - 1].time) < 3600000);
        if (hasSubHourly) info = PriceProvider.aggregateTo(info, 60);
      }
  ...
  /**
   * Fetch and parse the price graph of a single Stekker region, in the resolution that region
   * happens to serve. Returns a sorted array of { time, price } without duplicates.
   */
  async _getRegionPrices(region, opts) {
    const { startDate, endDate, forecast } = opts;
    const unit = 'MWh';
    ...
    try {
      ...
      throw new Error(`Failed to parse Stekker JSON data: ${e.message}`);
    }
