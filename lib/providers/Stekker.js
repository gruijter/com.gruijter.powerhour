...
const biddingZonesMap = {
  ...
  '10YRO-TEL------P': ['RO-3600'], // RO_Romania
...
class Stekker extends PriceProvider {
  ...
  async getPrices(options) {
    try {
      ...
      const { forecast, resolution } = opts;

      if (!zone || !biddingZonesMap[zone]) throw Error('biddingZone not supported by Stekker');
      // Format dates as YYYY-MM-DD.
      // Subtract/Add 12 hours to ensure we cover the full local day regardless of timezone (UTC-1 to UTC+3)
      // This ensures we request the calendar day containing the start/end time.
      const startDate = new Date(start.getTime() - 43200000).toISOString().split('T')[0];
      const endDate = new Date(end.getTime() + 43200000).toISOString().split('T')[0];
        ...
        if (!hasSubHourly) {
          if (forecast) {
            ...
            info.forEach((i) => {
              for (let j = 0; j < 2; j += 1) {
                newInfo.push({
                  time: new Date(i.time.getTime() + (j * 30 * 60 * 1000)),
                  price: i.price,
                });
              }
            });
  ...
  async _makeRequest(actionPath, timeout) {
    try {
      ...
      if (!result.ok) {
        ...
        throw new Error(`HTTP request Failed. Status Code: ${result.status} ${text.slice(0, 200)}`);
      }
