...
class SumMeterDevice extends Device {
  ...
  async onInitDevice() {
    try {
      ...
      if (this.settings.source_device_type === 'Homey device') {
        ...
        if (!sourceDeviceExists) throw Error(this.homey.__('source_device_missing_retry_named', { name: this.getName() }));
  ...
  async restartDevice(delay) {
    ...
    await setTimeoutPromise(dly).then(() => this.onInitDevice().catch((err) => this.error(err)));
  }
  ...
  async setTariffGroup(group) {
    ...
    await this.setSettings({ tariff_update_group: group }).catch((err) => this.error(err));
  ...
  async minMaxReset(reset, source) {
    ...
    await this.setCapability('last_minmax_reset', `${date} ${time}`).catch((err) => this.error(err));
