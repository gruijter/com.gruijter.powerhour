...
class DapFlows {
  ...
  getPricesBefore(args) {
    ...
    // Window length fallback: period -> hours
    ...
    const startStep = endStep - (Number(periodArg) * this.stepsPerHour);
  ...
  checkAvgRank(prices, hours, currentIndex, type) {
    const safeHours = Number.isFinite(Number(hours)) ? Number(hours) : 1;
    const durationSteps = Math.max(1, Math.round(safeHours * this.stepsPerHour));
    const avgPrices = [];
  ...
  async price_lowest(args) {
    ...
    if (pArg !== 'this_day' && Number.isFinite(Number(pArg))) minimum = Math.min(...this.device.state.pricesNext8h.slice(0, Number(pArg) * this.stepsPerHour));
  ...
  async price_lowest_avg_before(args) {
    ...
    const data = this.getPricesBefore(args);
    if (!data) return false;
    // Average duration fallback: duration -> hours
    const avgArg = args.duration ?? args.hours ?? 1;
    return this.checkAvgRank(data.prices, avgArg, data.relativeIndex, 'min');
  }
  ...
  async price_lowest_avg_between(args) {
    ...
    // Average duration fallback: duration -> hours
    const avgArg = args.duration ?? args.hours ?? 1;
    return this.checkAvgRank(prices, avgArg, currentIndex, 'min');
  }
  ...
  async price_highest_avg_between(args) {
    ...
    // Average duration fallback: duration -> hours
    const avgArg = args.duration ?? args.hours ?? 1;
    return this.checkAvgRank(prices, avgArg, currentIndex, 'max');
  }
  ...
  async price_lowest_next_hours(args) {
    ...
    const period = pArg !== undefined ? Number(pArg) : 99;
  ...
  async price_lowest_next_known_hours(args) {
    return this.price_lowest_next_hours(args);
  }
  ...
  async price_lowest_avg(args) {
    ...
    // Average duration fallback: duration -> hours
    const avgArg = args.duration ?? args.hours ?? 1;
    return this.checkAvgRank(pricesTotalPeriod, avgArg, currentStepIndex, 'min');
  }
  ...
  async price_highest(args) {
    ...
    // period acts as both string flag and numeric limit here
    const pArg = args.period ?? args.hours ?? 'this_day';
    if (pArg !== 'this_day' && Number.isFinite(Number(pArg))) maximum = Math.max(...this.device.state.pricesNext8h.slice(0, Number(pArg) * this.stepsPerHour));
  ...
  async price_highest_next_known_hours(args) {
    return this.price_highest_next_hours(args);
  }
  ...
  async price_highest_avg(args) {
    ...
    // Average duration fallback: duration -> hours
    const avgArg = args.duration ?? args.hours ?? 1;
    return this.checkAvgRank(pricesTotalPeriod, avgArg, currentStepIndex, 'max');
  }
  ...
  async price_highest_avg_before(args) {
    ...
    // Average duration fallback: duration -> hours
    const avgArg = args.duration ?? args.hours ?? 1;
    return this.checkAvgRank(data.prices, avgArg, data.relativeIndex, 'max');
  }
