...
class DapFlows {
  ...
  checkAvgRank(prices, hours, currentIndex, type) {
    const durationSteps = Number(hours);
    const avgPrices = [];
  ...
  async price_lowest_today(args) {
    if (!this.device.state || !this.device.state.pricesThisDay) throw Error('No prices available');
    const lowestNPrices = [...this.device.state.pricesThisDay].sort((a, b) => a - b).slice(0, args.number);
  ...
  async price_lowest_before(args) {
    ...
    const lowestNPrices = data.prices.sort((a, b) => a - b).slice(0, args.number);
    return this.device.state.priceNow <= Math.max(...lowestNPrices);
  }
  ...
  async price_lowest_between(args) {
    ...
    const lowestNPrices = [...prices].sort((a, b) => a - b).slice(0, args.number);
    return this.device.state.priceNow <= Math.max(...lowestNPrices);
  }
  ...
  async price_highest_between(args) {
    ...
    const highestNPrices = [...prices].sort((a, b) => b - a).slice(0, args.number);
  ...
  async price_lowest_avg_between(args) {
    ...
    if (!prices || prices.length === 0 || !isNowInWindow) return false;
    return this.checkAvgRank(prices, args.duration, currentIndex, 'min');
  }

  async price_highest_avg_between(args) {
    const { prices, isNowInWindow, currentIndex } = this.getPricesInWindow(args);
    if (!prices || prices.length === 0 || !isNowInWindow) return false;
    return this.checkAvgRank(prices, args.duration, currentIndex, 'max');
  }
  ...
  async price_highest_today(args) {
    if (!this.device.state || !this.device.state.pricesThisDay) throw Error('No prices available');
    const highestNPrices = [...this.device.state.pricesThisDay].sort((a, b) => a - b).reverse().slice(0, args.number);
    return this.device.state.priceNow >= Math.min(...highestNPrices);
  }
  ...
  async price_highest_before(args) {
    ...
    const data = this.getPricesBefore(args);
    if (!data) return false;
    const highestNPrices = data.prices.sort((a, b) => a - b).reverse().slice(0, args.number);
    return this.device.state.priceNow >= Math.min(...highestNPrices);
  }
