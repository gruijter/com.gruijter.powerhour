...
// precision of the prices token of the new_prices trigger
const roundTokenPrice = (price) => Math.round(price * 10000) / 10000;
...
class DapFlows {
  ...
  getPricesBefore(args) {
    const now = new Date();
    const nowLocal = TimeHelpers.toLocalDate(now, this.device.timeZone);
  ...
  getPricesInWindow(args) {
    ...
    // toLocaleString() drops sub-second precision, so strip it from "now" too before diffing -
    // otherwise the offset (and everything derived from it below) carries a few hundred ms of
    // residue (same bug class as TimeHelpers.getLocalMidnightUTC, fixed 2026-08-11).
    const now = new Date(rawNow.getTime() - (rawNow.getTime() % 1000));
    ...
    const time1Str = parseTime(time1);
    const time2Str = parseTime(time2);
    ...
    // Calculate UTC start and end using DST-aware conversion
    const getUTC = (localDate) => {
      const estimatedUTC = new Date(localDate.getTime() - homeyOffset);
      const checkLocal = TimeHelpers.toLocalDate(estimatedUTC, timeZone);
      const diff = localDate.getTime() - checkLocal.getTime();
      return new Date(estimatedUTC.getTime() + diff);
    };

    const startUTC = getUTC(startLocal);
    const endUTC = getUTC(endLocal);
  ...
  findBestAvgWindow(prices, durationSteps, type) {
    ...
    if (avgPrices.length === 0) return null;
  ...
  checkAvgRank(prices, durationSteps, currentIndex, type) {
    const best = this.findBestAvgWindow(prices, durationSteps, type);
    if (!best) return false;
    return (currentIndex >= best.idxMin) && (currentIndex <= best.idxMax);
  }

  // EXECUTORS FOR ACTION FLOWS
  async time_until_cheapest(args) {
    ...
    const lookaheadHours = Number(args.lookahead) || 24;
    const limitSteps = Math.max(durationSteps, Math.round(lookaheadHours * this.stepsPerHour));

    const prices = [...this.device.state.pricesNextHours].slice(0, limitSteps);
    if (prices.length < durationSteps) throw Error('Not enough price data available for the requested lookahead');

    const best = this.findBestAvgWindow(prices, durationSteps, 'min');
    if (!best) throw Error('Could not determine the cheapest window');

    const minutesUntil = best.idxMin * this.device.priceInterval;
    const hoursUntil = Math.round((minutesUntil / 60) * 100) / 100;

    const { periodStart } = this.device.getUTCPeriods();
    const startUTCms = periodStart.getTime() + (best.idxMin * this.device.priceInterval * 60 * 1000);
    const startLocal = TimeHelpers.toLocalDate(new Date(startUTCms), this.device.timeZone);
    const pad = (n) => String(n).padStart(2, '0');
    const startTime = `${pad(startLocal.getHours())}:${pad(startLocal.getMinutes())}`;

    return {
      minutesUntil,
      hoursUntil,
      startTime,
      avgPrice: Math.round(best.avgPrice * 10000) / 10000,
    };
  }
  ...
  async prices_json(args) {
    ...
    const roundedPrices = prices.map(roundTokenPrice);
    const priceString = JSON.stringify(roundedPrices);
    const provider = this.device.getProviderForPeriod(period);
  ...
  async set_export_variable_markup(args) {
    const val = args.value;
    this.device.log('changing export variable markup via flow', this.device.getName(), val);
    await this.device.setSettings({ export_variableMarkup: val }).catch((err) => this.device.error(err));
  }

  async set_export_fixed_markup(args) {
    const val = args.value;
    this.device.log('changing export fixed markup via flow', this.device.getName(), val);
    await this.device.setSettings({ export_fixedMarkup: val }).catch((err) => this.device.error(err));
  }

  async set_export_fixed_markup_TOD(args) {
    const val = args.value;
    this.device.log('changing export Time Of Day markup via flow', this.device.getName(), val);
    const todObject = PriceCalculator.parseTOD(val); // will throw Error if invalid
    if (todObject === null) await this.device.setSettings({ export_fixedMarkupTOD: '' }).catch((err) => this.device.error(err));
    else await this.device.setSettings({ export_fixedMarkupTOD: val }).catch((err) => this.device.error(err));
  }

  async set_export_fixed_markup_weekend(args) {
    const val = args.value;
    this.device.log('changing export Weekend markup via flow', this.device.getName(), val);
    if (!Number.isFinite(val)) throw Error('Value is not a number');
    await this.device.setSettings({ export_fixedMarkupWeekend: val }).catch((err) => this.device.error(err));
  }
  ...
  async new_prices_received(args) {
    ...
    const priceString = JSON.stringify(roundedPrices);
    const provider = this.device.getProviderForPeriod(period);
    const isForecast = provider.includes('(forecast)');
    const tokens = { prices: priceString, provider };
    const state = { period, isForecast };
    this.device.log(`${this.device.getName()} received new prices for ${period} from ${provider} (${roundedPrices.length} slots)`);
  ...
  async checkNewMarketPrices(oldPrices, newPrices, period, periods) {
    const { start, end } = PriceCalculator.getPeriodRange(periods, period);
    ...
    // Compare what the flow token exposes (rounded markup prices), not the raw market prices: a
    // higher ranking provider confirming the same prices differs from the previous source only
    // below token precision, and must not trigger the flow again. Its provider tag is still
    // stored by mergeByAuthority(), so the provider token of later triggers is up to date.
    const muPrices = PriceCalculator.calculateMarkupPrices(newPricesSelection, this.device.settings, this.device.timeZone);
    const prices = PriceCalculator.selectPrices(muPrices, start, end);
    const oldMuPrices = PriceCalculator.selectPrices(oldPricesSelection, start, end);
    const samePrices = prices.length === oldMuPrices.length
      && prices.every((price, index) => Number.isFinite(oldMuPrices[index]) && roundTokenPrice(price) === roundTokenPrice(oldMuPrices[index]));

    if (!samePrices) {
      await this.new_prices_received({ prices, period });
    } else {
      this.device.log(`${this.device.getName()} prices for ${period} unchanged (source: ${this.device.getProviderForPeriod(period)})`);
    }
  }
}
