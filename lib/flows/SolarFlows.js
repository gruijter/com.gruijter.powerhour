...
class SolarFlows extends SumFlows {
  ...
  async solar_json(args) {
    ...
    for (let t = start; t < end; t += 15 * 60 * 1000) {
      ...
      // Use toLocaleString to get the real local slot index from the real UTC timestamp
      const localT = new Date(new Date(t).toLocaleString('en-US', { timeZone: timezone }));
  ...
  async solar_production_enters_top(args) {
    ...
    // Iterate the 96 local slots of today using real UTC timestamps → local slot index via toLocaleString
    const todayYields = [];
    ...
    for (let i = 0; i < 96; i++) {
      ...
      const slotLocal = new Date(new Date(tUTC).toLocaleString('en-US', { timeZone: timezone }));
      const localSlotIndex = (slotLocal.getHours() * 4) + Math.floor(slotLocal.getMinutes() / 15);
      const yf = this.device.yieldFactors[localSlotIndex] !== undefined ? this.device.yieldFactors[localSlotIndex] : 0;
      const power = Math.round(rad * yf);
      todayYields.push({ slot: localSlotIndex, power });
    }
