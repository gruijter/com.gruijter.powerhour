...
class GridFlows extends SumFlows {
  ...
  async load_json(args) {
    ...
    const values = LoadForecastStrategy.getLoadSeries(this.device.weeklyProfile, start, end, timezone);
    const net = this.device.getNetForecast ? this.device.getNetForecast(start, end) : null;

    const accuracy = this.device.getForecastAccuracy ? this.device.getForecastAccuracy() : {};
    return {
