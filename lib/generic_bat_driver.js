...
class BatDriver extends Driver {
  ...
  async startPollingEnergy(interval) {
    const int = interval || 1; // 1 second for realtime-ish updates
    if (this.intervalIdEnergyPoll) {
      this.homey.clearInterval(this.intervalIdEnergyPoll);
      this.homey.clearTimeout(this.intervalIdEnergyPoll);
    }
    ...
    let retries = 0;
    while (!this.homey.app.api && retries < 60) {
      await setTimeoutPromise(1000);
      retries += 1;
      if (this.isDestroyed) return;
    }
    if (!this.homey.app.api) {
      this.error('Homey API not ready, cannot start energy polling');
      return;
    }
  ...
  async processEnergyLogic(cumulativePower, interval) {
    ...
    console.log(`[BatDriver] P1: ${cumulativePower}W | Interval: ${interval.toFixed(2)}s | Smoothing: ${smoothing}% | MinLoad: ${minLoad}W | Offset: ${x}W | Samples: ${samples}`);
    const devices = await this.getDevices();
    ...
    devices.forEach((device) => {
      ...
      device.triggerXOMFlow(strat, samples, x, smoothing, minLoad);
    });
  }
