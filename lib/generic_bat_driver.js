...
class BatDriver extends Driver {
  ...
  async checkAndPollDevice(device) {
    ...
    if (!sourceDeviceExists) {
      ...
      await device.setUnavailable('Source device is missing. Retry in 10 minutes.').catch((err) => this.error(err));
  ...
  async checkDeviceHealth(device) {
    ...
    if (!sourceDeviceExists) {
      ...
      await device.setUnavailable('Source device is missing. Retrying ..').catch((err) => this.error(err));
