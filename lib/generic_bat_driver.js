...
class BatDriver extends Driver {
  ...
  collectBatteryInfo(devices) {
    return devices
      .map((device) => ({
        ...
        soc: device.soc,
        actualPower: -device.getCapabilityValue('measure_watt_avg'), // Invert: Device is Charge(+)/Discharge(-). Driver logic needs Charge(-)/Discharge(+)
  ...
  triggerXOMFlows(devices, strategy, samples, x, smoothing, minLoad) {
    devices.forEach((device) => {
      ...
      const tokens = {
        power: Math.round(-device.xomTargetPower), // Invert: Driver logic is Charge(-)/Discharge(+). Flow needs Charge(+)/Discharge(-)
        x,
        smoothing,
        minLoad,
      };
