...
// netSeries: optional forecast grid exchange (W) per 15-minute slot from startTm, null = not drawn.
const getGridForecastChart = async (weeklyProfile, startTm, endTm, title, realPowerData = [], timezone = 'UTC', isToday = false, netSeries = null) => {
  ...
  const netData = times.map((_, i) => (Array.isArray(netSeries) && typeof netSeries[i] === 'number' ? netSeries[i] : null));
  const netValues = netData.filter((v) => v !== null);
  const hasNet = netValues.length > 0;
  if (hasNet) maxVal = Math.max(maxVal, ...netValues);
  const minNet = hasNet ? Math.min(0, ...netValues) : 0;
    ...
    data: {
      ...
      datasets: [
        ...
        ...(hasNet ? [{
          ...
          borderColor: 'rgba(255, 170, 60, 0.9)',
          borderWidth: 2,
          borderDash: [6, 4],
          pointRadius: 0,
          data: netData,
        ...
        yAxes: [
          {
            ticks: {
              ...
              beginAtZero: true,
              suggestedMax: yMax,
              ...(minNet < 0 ? { suggestedMin: Math.floor((minNet * 1.1) / 100) * 100 } : {}),
            },
            gridLines: {
              color: 'rgba(255, 255, 255, 0.2)',
            },
