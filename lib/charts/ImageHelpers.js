...
const imageUrlToStream = async (input, stream, context) => {
  ...
  const cacheObject = imageCache.find((object) => object.key === cacheKey);
  if (cacheObject) {
  ...
  const fetchPromise = executeWithConcurrencyLimit(async () => {
    ...
    if (useHomey) {
      ...
      timeoutId = setTimeout(abort, 10000); // eslint-disable-line homey-app/global-timers -- fallback without a Homey instance; cleared in finally
    }
    ...
    try {
      ...
      const newCacheObject = { key: cacheKey, buffer };
      imageCache.push(newCacheObject);
