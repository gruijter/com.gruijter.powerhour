...
const getSolarChart = async (forecastData, yieldFactors, startTm, endTm, title, realPowerData = [], timezone = 'UTC') => {
  ...
  console.log(`[SolarChart] Generating ${title}. Start: ${startTm.toISOString()}, End: ${endTm.toISOString()}`);
  console.log(`[SolarChart] RealPowerData input length: ${realPowerData.length}`);
  ...
  console.log(`[SolarChart] Relevant power data: ${relevantPowerData.length}`);
  if (relevantPowerData.length > 0) {
    console.log(`[SolarChart] First relevant: ${new Date(relevantPowerData[0].time).toISOString()} ${relevantPowerData[0].power}W`);
  }
  ...
  const nonNulls = realData.filter((v) => v !== null).length;
  console.log(`[SolarChart] Chart points generated: ${realData.length}. Non-null real values: ${nonNulls}`);
