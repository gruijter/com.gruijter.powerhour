...
const { hourLabels, thinLabels, nowLineAnnotation } = require('./ChartHelpers');
...
const getSolarChart = async (forecastData, yieldFactors, startTm, endTm, title, realPowerData = [], timezone = 'UTC', absoluteMaxPower = 0, isToday = false) => {
  ...
  const annotations = (isToday && times.length > 0) ? nowLineAnnotation(times[0], 15, data.length) : {};
...
const getDistributionChart = async (yieldFactors, title, timezone = 'UTC', trainingConfidence = null, datasetLabels = {}) => {
  const yieldFactorLabel = datasetLabels.yieldFactor || 'Yield Factor';
  const modelFitLabel = datasetLabels.modelFit || 'Model Fit (%)';
  if (!yieldFactors || yieldFactors.length === 0) return null;

  const labels = [];
  const data = [];
  // Before the first nightly retrain populates real confidence values, trainingConfidence is a
  // freshly-initialized all-zero array (see device.js) - drawing that as a "dashed line" just
  // pins it flat on the 0% gridline, indistinguishable from no line at all. Treat it as absent
  // rather than show a misleading axis/legend for a line nobody can actually see.
  const hasConfidence = Array.isArray(trainingConfidence) && trainingConfidence.some((v) => v > 0);
  const confidenceData = hasConfidence ? [] : null;
  ...
  const thinnedLabels = thinLabels(labels);
        ...
        yAxes: [
          {
            ...
            // Chart.js draws this as part of the axis' own layout box, so - unlike a top-level
            // title with hand-guessed space padding - it is always pixel-aligned to the axis it
            // labels, regardless of tick-label width or chart size.
            ...(confidenceData ? {
              scaleLabel: {
                display: true,
                labelString: yieldFactorLabel,
