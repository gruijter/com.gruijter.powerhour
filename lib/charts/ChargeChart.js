...
const getChargeChart = async (
  ...
  try {
    ...
    const targetLength = Math.max(marketLength, Object.keys(scheme).length);
    for (let i = 0; i < targetLength; i += 1) {
      if (!scheme[i]) {
        scheme[i] = {
          power: 0, duration: 0, soc: null, price: null, isForecast: true, isFuture: true,
        };
      }
    }
    ...
    Object.keys(scheme).forEach((idx, index) => {
      ...
      // SoC real-vs-forecast is about whether the slot has actually happened yet (isFuture),
      // not whether the underlying price is a forecast (isForecast) - those are independent.
      // Fall back to isForecast for callers that haven't been updated to set isFuture.
      const isFuture = (typeof hour.isFuture === 'boolean' ? hour.isFuture : hour.isForecast) || index >= marketLength;
    ...
    const startTimestamp = startMs instanceof Date ? startMs.getTime() : startMs;
    const labels = hourLabels(slotTimes(startTimestamp, prices.length, interval), timeZone);
    ...
    // Charge/discharge bars wash out based on isFuture (plan not yet executed); the plain
    // no-power price bars wash out based on isForecast (price itself not yet a real/published
    // value) - these are independent signals, don't conflate them.
    const backgrounds = Object.keys(scheme).map((idx, index) => {
      const hour = scheme[idx];
      const isFuture = (typeof hour.isFuture === 'boolean' ? hour.isFuture : hour.isForecast) || index >= marketLength;
      const priceIsForecast = hour.isForecast || index >= marketLength;
      if (hour.power > 0) { // charging
        const chargeEnergy = hour.power * (hour.duration / 60);
        const g = 255 - 100 * (chargeEnergy / maxChargePower);
        return isFuture ? `rgba(0, ${Math.round(g)}, 0, 0.45)` : `rgb(0,${Math.round(g)},0)`;
      }
      if (hour.power < 0) { // discharging
        ...
        return isFuture ? `rgba(${Math.round(r)}, 50, 20, 0.45)` : `rgb(${Math.round(r)},50,20)`;
      }
      if (priceIsForecast) return 'rgba(210,210,210,0.45)'; // washed-out light grey
    ...
    const chart = {
      ...
      options: {
        ...
        plugins: {
          ...
          annotation: {
            annotations: isToday ? nowLineAnnotation(startTimestamp, interval, prices.length) : {},
          },
          title: {
            display: true,
            text: (() => {
