...
const getChargeChart = async (strategy, startHour = 0, marketLength = 99, maxChargePower = 2200, maxDischargePower = 1700, interval = 60, exportPrices = []) => {
  try {
    ...
    const offsetData = prices.map((val, i) => {
      if (i === maxIndex) return 15;
      if (i === minIndex) {
        if (val < 0) return 15;

        const range = Math.ceil(120 / interval);
        ...
        const pixelOffset = relativeDiff * 150;
      ...
      data: {
        ...
        datasets: [
          ...
          },
          {
            ...
            borderWidth: 2,
            backgroundColor: 'rgba(20, 60, 140, 0.5)',
            pointRadius: 0,
            fill: true,
      ...
      options: {
        ...
        scales: {
          ...
          yPower: {
            position: 'right',
            min: 0,
            suggestedMax: Math.max(3.7, ...plannedPowerData, ...(actualPowerData.filter((v) => typeof v === 'number'))) * 1.25,
            ticks: {
              font: { size: 14 },
