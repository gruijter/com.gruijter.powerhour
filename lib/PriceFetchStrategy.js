...
class PriceFetchStrategy {
  ...
  static shouldSkipFetch(marketPrices, periods, isNordpoolZone, currentProvider, primaryProviderName) {
    ...
    if (hasTomorrow) {
      ...
      // For non-Nordpool zones, don't skip entirely but only poll the primary provider for intraday updates
      if (currentProvider && currentProvider === primaryProviderName) {
        return { skip: false, primaryOnly: true, hasTomorrow };
      }
  ...
  static async fetchPrices(providers, periods, resolution, validator, logger, primaryOnly = false) {
    let newMarketPrices = null;

    let providersToUse = providers;
    if (primaryOnly && providers.length > 0) {
      providersToUse = [providers[0]];
    }
