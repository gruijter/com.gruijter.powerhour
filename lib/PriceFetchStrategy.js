...
class PriceFetchStrategy {
  ...
  static shouldSkipFetch(marketPrices, periods, isNordpoolZone) {
    if (!marketPrices || marketPrices.length === 0) return { skip: false };
    ...
    // For non-Nordpool zones, we might have fetched early Nordpool prices (fallback).
    // We want to keep trying until 23:00 to ensure we get the official Entsoe prices (market decoupling).
    if (!isNordpoolZone && hasTomorrow && nowBrussels.getHours() >= 23) {
      return { skip: true, reason: 'Already have tomorrow prices (after 23:00)' };
    }
