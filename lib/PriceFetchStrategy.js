...
class PriceFetchStrategy {
  ...
  static shouldSkipFetch(marketPrices, periods, isNordpoolZone) {
    ...
    // We want to keep trying until 14:00 to ensure we get the official Entsoe prices (market decoupling).
    if (!isNordpoolZone && hasTomorrow && nowBrussels.getHours() >= 14) {
      return { skip: true, reason: 'Already have tomorrow prices (after 14:00)' };
    }
