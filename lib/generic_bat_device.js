...
class batDevice extends Device {
  ...
  async updatePrices(pricesNextHours, pricesNextHoursMarketLength, priceInterval, pricesNextHoursIsForecast) {
    try {
      ...
      await this.setStoreValue('pricesNextHoursIsForecast', pricesNextHoursIsForecast).catch(this.error);
  ...
  async updateChargeChart() {
    ...
    if (strategy) {
      await this.setCapability('roi_duration', strategy.duration).catch(this.error);
  ...
  async updateValue(val, cap) {
    try {
      ...
      if (cap === 'soc') {
        ...
        await this.setCapability('meter_kwh_stored', storedkWh).catch(this.error);
      }
  ...
  async updateMeters({ ...reading }, { ...periods }) {
    await this.setCapability('meter_power_hidden', reading.meterValue).catch(this.error);
