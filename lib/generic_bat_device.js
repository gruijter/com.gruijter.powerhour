...
class BatDevice extends Device {
    ...
    try {
      ...
      if (this.getSettings().roiEnable) {
        this.homey.setTimeout(async () => {
          await setTimeoutPromise(10000 + (Math.random() * 10000), this).catch((err) => this.error(err));
          if (this.sessionId !== currentSessionId) return;
          if (this.pricesNextHours) {
            this.flows.triggerNewRoiStrategyFlow().catch((err) => this.error(err));
  ...
  async updateChargeChart() {
    ...
    if (strategy) {
      ...
      const urlNextHours = await getChargeChart(strategy, startHour, this.pricesNextHoursMarketLength, this.getSettings().chargePower, this.getSettings().dischargePower, this.priceInterval);
