...
class batDevice extends Device {
  ...
  async onInitDevice() {
    try {
      ...
      if (this.getSettings().roiEnable) {
        ...
        await this.triggerNewRoiStrategyFlow().catch((err) => this.error(err));
        await setTimeoutPromise(20000 + (Math.random() * 10000)).catch((err) => this.error(err));
        if (this.sessionId !== currentSessionId) return;
  ...
  async onSettings({ newSettings, changedKeys }) {
    ...
    if (changedKeys.includes('roiEnable')) {
      const HP2023 = this.homey.platformVersion === 2;
      if (!HP2023 && newSettings.roiEnable) throw Error(this.homey.__('error_roi_hp2023'));
    }
    setTimeout(() => {
  ...
  async priceBattBestTrade(args) {
    ...
    if (!this.pricesNextHours) throw Error(this.homey.__('error_no_prices'));
  ...
  async findRoiStrategy(args) {
    try {
      ...
      if (!this.pricesNextHours) throw Error(this.homey.__('error_no_prices'));
      if (!this.priceInterval) throw Error(this.homey.__('error_no_price_interval'));
  ...
  async triggerNewRoiStrategyFlow() {
    try {
      ...
      uniqueArgs.forEach(async (args) => {
        const tokens = await this.findRoiStrategy(args).catch((err) => this.error(err));
        if (tokens) {
          const state = args;
          this.homey.app.triggerNewRoiStrategy(this, tokens, state);
          await this.reTriggerNewRoiStrategyFlow(tokens, args).catch((err) => this.error(err));
        }
      });
      await setTimeoutPromise(5000 + Math.random() * 20000);
      return Promise.resolve(true);
  ...
  async reTriggerNewRoiStrategyFlow(tokens, args) {
    try {
      ...
      await setTimeoutPromise(delay).catch((err) => this.error(err));
      if (this.sessionId !== currentSessionId) return;
      const state = args;
      const newTokens = {
  ...
  async updateChargeChart() {
    ...
    const strategy = await this.findRoiStrategy({ minPriceDelta }).catch((err) => this.error(err));
    if (strategy) {
