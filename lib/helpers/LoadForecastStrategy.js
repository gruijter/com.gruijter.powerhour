...
const processHistoricData = ({
  ...
  let updatedSlots = 0;
  const slotLabel = (slot) => `${String(Math.floor(slot / 4)).padStart(2, '0')}:${String((slot % 4) * 15).padStart(2, '0')}`;
  ...
  for (let day = 0; day < 7; day++) {
    for (let slot = 0; slot < 96; slot++) {
      ...
      // TEMP DIAGNOSTIC - was gated to slot < 20 (night only); widened to every trained slot
      // to see the actual per-hour values behind the flat-afternoon forecast (coverage itself
      // turned out complete per device.js's logCoverageHistogram - only ~24/96 slots per day
      // are ever directly trained anyway, the rest interpolated, so this stays bounded).
      // Revert to the slot < 20 gate once the root cause is found.
      logger(`[Diag] Day ${day} Slot ${slot} (${slotLabel(slot)}) | `
  ...
  // TEMP DIAGNOSTIC - log which day/slot ranges got zero real samples (i.e. are about to be
  // bridged by the straight-line interpolation below) - pairs with device.js's
  // logCoverageHistogram() for the upstream half of this trace (raw source data coverage).
  // Remove once the flat-afternoon-forecast root cause is found.
  const dayNames = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  for (let day = 0; day < 7; day++) {
    const trainedCount = trainedSlots[day].filter(Boolean).length;
    if (trainedCount === 0) {
      logger(`[Diag] Day ${day} (${dayNames[day]}): 0/96 slots trained - no real samples at all for this day-of-week.`);
    } else {
      const emptyRanges = [];
      let rangeStart = null;
      for (let slot = 0; slot < 96; slot++) {
        if (!trainedSlots[day][slot]) {
          if (rangeStart === null) rangeStart = slot;
        } else if (rangeStart !== null) {
          emptyRanges.push(`${slotLabel(rangeStart)}-${slotLabel(slot - 1)}`);
          rangeStart = null;
        }
      }
      if (rangeStart !== null) emptyRanges.push(`${slotLabel(rangeStart)}-${slotLabel(95)}`);
      logger(`[Diag] Day ${day} (${dayNames[day]}): ${trainedCount}/96 slots trained; `
        + `empty (interpolated) ranges: ${emptyRanges.length ? emptyRanges.join(', ') : 'none'}`);
    }
  }
...
const calculateForecast = ({
  ...
  // DST-aware local midnight to UTC conversion using the same two-pass method as TimeHelpers.js
