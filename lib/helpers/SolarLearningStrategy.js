...
const processHistoricData = ({
  ...
  // KNOWN LIMITATION (2026-08-14): clear-sky detection is weak-to-absent at the
  // morning/evening edges of the day, and this is a real, currently-unsolved gap -
  // not just theoretical. Confirmed on a real device (Leo's east array): on
  // 2026-08-13, the 05:00 UTC slot's actual yieldFactor (164.1) was the single
  // highest of the entire 13-day training window (median 71.5, i.e. more than 2x
  // the typical value) - an exceptionally clear morning - yet the ramp-region branch
  // below gives it no extra weight at all, so the trained forecast undershot that
  // day by design, not by bug.
  //
  // What each region actually does today:
  //   - Ramp region (below, maxSlotRad < MIN_RADIATION_FOR_ROBUST_STATS, i.e.
  //     roughly the first/last ~3-4h of daylight): plain median of every sample
  //     that clears a near-nonexistent gate (radiation > 10 W/m2, or > 5% of the
  //     slot's historic max) - a hazy morning and a perfectly clear one count as
  //     exactly one vote each.
  //   - Normal region (else branch below): "top 25% by power, then median" is a
  //     binary in/out clear-sky proxy, not graduated - the 25th-percentile day and
  //     the single clearest day both get one vote once inside the cutoff. It's also
  //     an imperfect proxy in its own right: power can rank a hazy-but-briefly-
  //     bright hour above a genuinely clear one when the radiation forecast that
  //     fed the ratio was itself off (see the OpenMeteo.fetchEra5 ERA5-blend
  //     addition, which mitigates - but only for data 5+ days old, so never the
  //     ramp region's most recent, often most informative days).
  //   - Nowhere in this file is there a true clear-sky index (comparing a sample to
  //     a theoretical/astronomical clear-sky irradiance curve for that exact
  //     lat/lon/date/time) - maxSlotRad is only an empirical "best ever seen in the
  //     training window" ceiling, and it's used solely as a pass/fail gate, never as
  //     a graduated weight.
  //
  // Research directions considered and why they don't (yet) clear the bar:
  //   1. Rank ramp-region samples by radiation-closeness-to-maxSlotRad instead of
  //      power (mirrors the normal-region approach). Rejected: radiation here is
  //      Open-Meteo model output, not a local measurement - using it as the primary
  //      "was it clear" signal means trusting the LESS reliable of the two channels
  //      over the array's own directly-measured power.
  //   2. Score each training DAY by how smooth/bell-shaped its own power curve is
  //      (a clear day should look like a single clean peak) and weight that day's
  //      samples accordingly across all slots, ramp region included. Rejected on two
  //      independent grounds: (a) a bell shape isn't a valid reference for a
  //      non-south array or one with fixed structural shading (e.g. a roof-ridge
  //      shadow producing a sharp, legitimate, repeatable step on an otherwise
  //      perfectly clear day - confirmed on Leo's west array); (b) it needs
  //      sub-hourly resolution to detect intra-day cloud noise at all, but 13 of the
  //      14 training days only have hourly Insights data (`last14Days`), which is
  //      already an hourly AVERAGE (see the timestamp-semantics fix elsewhere in
  //      this file) - exactly the kind of within-hour variability a smoothness
  //      score would need is already averaged away for all but the most recent 24h.
  //   3. Residual-based: score a day's clearness by how closely its observations
  //      track what the array's OWN already-trained model would have predicted for
  //      that day (orientation- and shading-aware by construction, since it reuses
  //      the model's own learned shape; works at hourly resolution since it compares
  //      scalars, not sub-hourly shape). Not rejected, but not attempted either: it
  //      has a real circularity risk - using the model to judge which of its own
  //      training inputs to trust can entrench an existing bias instead of
  //      correcting it, and would need a careful bootstrap/robustness design
  //      (e.g. only apply once an initial fit has stabilized, cap how much any
  //      single day's agreement/disagreement can swing the weighting) before it's
  //      safe to try. This is the most promising open direction if this gets picked
  //      back up.
  slotSamples.forEach((samples, idx) => {
    ...
    // All the limits above are derived from (or ratchet against) the highest-POWER
    // samples, which for any tilted/oriented array are typically the midday ones - and
    // midday is systematically where yieldFactor is at its LOWEST (GHI itself peaks at
    // midday, while panel-plane irradiance / GHI does not). Applying that as a flat
    // ceiling to every slot wrongly clamps the genuinely higher yieldFactor near
    // sunrise/sunset (confirmed against a real east-facing array: true yieldFactor
    // ~76 at sunrise vs. ~3-9 at midday, clamped down to ~9 everywhere by this cap).
    // Loosen it with a per-slot physical ceiling anchored to the array's own known
    // peak capacity, evaluated at THIS sample's own radiation - the same kind of check
    // already used above to reject raw power samples above peakPower*1.1. This never
    // makes the cap tighter than before (Math.max), so it can only stop wrongly
    // clamping legitimate ramp-hour values, not weaken protection against actually
    // impossible ones.
...
const mergeYields = ({
  historicYields, liveYields, alpha = 0.7, limit = 0, peakPower = 0,
}) => {
  // Same physically-anchored backstop as processHistoricData's per-slot safety cap: `limit`
  // (Step 1's datasetMaxYF) is derived from the highest-POWER samples, which for any
  // non-flat array are typically midday - and midday is systematically where yieldFactor is
  // LOWEST (GHI itself peaks at midday, panel-plane/GHI ratio does not). Using it as a flat
  // ceiling here wrongly clamped every slot back down to the midday value even after
  // processHistoricData's own per-slot cap had correctly let a higher sunrise/sunset
