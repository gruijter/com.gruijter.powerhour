...
const getStrategy = ({
  ...
  if (peakPower > 0) {
    ...
    if (power > peakPower) power = peakPower; // Cap to exact limit
  }
...
const detectCurtailment = ({
  ...
  if (expectedPower > triggerExpected && currentPower < offThreshold && lastPower > wasRunningThreshold) {
    ...
    // Absolute bypass: undeniably online se se if producing > 10% of peak (min 250W)
...
const processHistoricData = ({
  ...
  const entriesWithYield = powerEntries.map((entry) => {
    ...
    if (peakPower > 0) {
      ...
      if (power > peakPower) power = peakPower; // Cap to exact hardware limit
    }
  ...
  // Calculate the 90th percentile of yields to establish a robust "Physical Limit",
  // ignoring extreme outliers like cloud lensing.
  let datasetMaxYF = 0;
  const validEntries = [];
...
const calculateForecast = ({
  ...
  // Instantaneous Power: use UTC slot index because yieldFactors[] are indexed by
  // local slot-as-UTC (the "local-slot" convention used throughout solar device).
  // The local slot index equals getUTCHours/getUTCMinutes when the timestamp is a
  // "fake-local" value (local time numerically stored as UTC ms).
  const forecastRadiation = getInterpolatedRadiation(t, forecastData);
  const localSlotIndex = (now.getUTCHours() * 4) + Math.floor(now.getUTCMinutes() / 15);
  const yieldFactor = yieldFactors[localSlotIndex] !== undefined ? yieldFactors[localSlotIndex] : 0;
  const expectedPower = Math.round(forecastRadiation * yieldFactor);

  // Day Total Yield
  let totalYield = 0;

  // Get the "fake-local" Day start: local 00:00 numerically stored as UTC ms.
  // We deliberately do NOT subtract the UTC offset here — the consumer of fakeLocalDayStartMs
  // uses getUTCHours() to read back the local hour, which only works if the timestamp
  // stays in "fake-local" space (local clock time = UTC numeric value).
  // This is consistent with how yieldFactors[] are indexed everywhere in the solar device.
  const getFakeLocalDayStart = (d) => {
    const localTimeAsDate = new Date(d.toLocaleString('en-US', { timeZone: timezone }));
    localTimeAsDate.setHours(0, 0, 0, 0); // set local 00:00 (fake-local midnight)
    return localTimeAsDate; // .getTime() = ms of local 00:00, numerically as if UTC
  };

  const fakeLocalDayStartMs = getFakeLocalDayStart(now).getTime();
  const nextDay = new Date(now);
  nextDay.setDate(nextDay.getDate() + 1);
  const fakeLocalDayEndMs = getFakeLocalDayStart(nextDay).getTime();

  for (let fakeLocalSlotMs = fakeLocalDayStartMs; fakeLocalSlotMs < fakeLocalDayEndMs; fakeLocalSlotMs += 15 * 60 * 1000) {
    // fakeLocalSlotMs holds the local slot time numerically as UTC ms
    // getUTCHours/getUTCMinutes read back local hours/minutes correctly
    const localSlot = (new Date(fakeLocalSlotMs).getUTCHours() * 4) + Math.floor(new Date(fakeLocalSlotMs).getUTCMinutes() / 15);
    const rad = getInterpolatedRadiation(fakeLocalSlotMs, forecastData);
    const yf = yieldFactors[localSlot] !== undefined ? yieldFactors[localSlot] : 0;
    const power = rad * yf;
