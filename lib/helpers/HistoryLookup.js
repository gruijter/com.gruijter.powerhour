...
const findClosestSample = (history, timeMs, windowMs, valueKey) => {
  ...
  // Single pass: this runs once per chart slot over histories of up to 2880 entries.
  let closest = null;
  let bestDist = windowMs;
  for (let i = 0; i < history.length; i += 1) {
    const dist = Math.abs(history[i].time - timeMs);
    if (dist < bestDist) {
      bestDist = dist;
      closest = history[i];
    }
...
// `maxPowerW` is the plausibility ceiling a derived sample must stay under. It defaults to the
// historical flat 30000 W so solar's call sites keep their exact previous behaviour - solar
// production is bounded by the array/inverter, not by the grid connection, so a connection-derived
// ceiling would be the wrong quantity there. The grid device passes its own connection-derived
// ceiling instead (see drivers/grid/device.js#getHomePowerCeilingW()).
const convertCumulativeToPower = (entries, maxPowerW = 30000) => {
  const powerEntries = [];
...
/**
 * Mean of all samples with startMs <= time < endMs, or null if there are none. For a rate such as
 * power, a slot's value is its average - a single point at the slot boundary lands on either side
 * of a transition that happens on the boundary, and moves it a whole slot.
 *
 * @param {Array<{time: number}>} history - time-series array, entries shaped { time, [valueKey]: number }
 * @param {number} startMs - window start (inclusive), Unix ms
 * @param {number} endMs - window end (exclusive), Unix ms
 * @param {string} valueKey - property name holding the numeric value
 * @returns {number|null}
 */
const averageSamplesInWindow = (history, startMs, endMs, valueKey) => {
  if (!Array.isArray(history) || history.length === 0) return null;
  let sum = 0;
  let count = 0;
  history.forEach((d) => {
    if (d && d.time >= startMs && d.time < endMs && typeof d[valueKey] === 'number') {
      sum += d[valueKey];
      count += 1;
    }
  });
...
module.exports = {
  findClosestSample, averageSamplesInWindow, fetchYesterdayAndToday, convertCumulativeToPower, smoothTimeSeries,
};
