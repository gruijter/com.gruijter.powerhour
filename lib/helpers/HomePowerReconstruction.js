 ...
 * @returns {Array<{t: string, y: number, grid: number}>} reconstructed home-power samples, ISO
 *   timestamp + Watts, plus the grid exchange (+ import / - export) the sample was derived from
 */
const combineComponentsToHomePower = ({
  ...
  applySmoothing = true,
  logger = () => {},
  // Upper plausibility clamp for a reconstructed home-load sample. Defaults to the historical
  // flat 30000 W; the grid device passes its own connection-derived ceiling so the clamp scales
  // with the actual grid connection. See lib/helpers/GridConnection.js.
  maxPowerW = 30000,
}) => {
  ...
  timestamps.forEach((t) => {
    ...
    reconstructed.push({
      ...
      y: Math.max(0, Math.min(maxPowerW, Math.round(homePower))),
