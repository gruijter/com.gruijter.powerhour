...
module.exports = {
  ...
  async _fetchInsightsPriceHistory(capName, cachedLogs = null) {
    ...
    const allLogs = cachedLogs || await api.insights.getLogs().catch(() => []);
  ...
  async _fetchDapPriceHistoryFromInsights() {
    const { api } = this.homey.app || {};
    const allLogs = api ? await api.insights.getLogs().catch(() => []) : [];
   ...
   * slot is not planned, so every 15 minutes of it gets the slot average. null where no plan
  ...
  getPlannedPowerSeries(startMs, endMs) {
    ...
    for (let t = startMs; t < endMs; t += 15 * 60 * 1000) {
      const idx = plan ? Math.floor((t - plan.startMs) / plan.intervalMs) : -1;
      const slot = plan && idx >= 0 ? plan.slots[idx] : null;
      values.push(slot ? Math.round(slot.power * Math.min(1, slot.duration / (plan.intervalMs / 60000))) : null);
    }
    return values;
  },
