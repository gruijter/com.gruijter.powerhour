...
class DapFlows {
  ...
  async prices_json(args) {
    ...
    if (!prices) throw Error(this.device.homey.__('error_no_prices'));
  ...
  async set_fixed_markup_weekend(args) {
    ...
    if (!Number.isFinite(val)) throw Error(this.device.homey.__('error_value_not_number'));
  ...
  async price_lowest(args) {
    if (!this.device.state || !this.device.state.pricesThisDay) throw Error(this.device.homey.__('error_no_prices'));
  ...
  async price_lowest_next_hours(args) {
    if (!this.device.state || !this.device.state.pricesNextHours) throw Error(this.device.homey.__('error_no_prices'));
