...
class SumMeterDevice extends Device {
  ...
  async initDeviceValues() {
    ...
    if (!this.tariffHistory) {
      this.tariffHistory = {
        ...
        currentExport: this.settings.tariff,
  ...
  async updateTariffHistory(tariff, currentTm, priceInterval, exportTariff) {
    try {
  ...
  async updateGridTariffs(currentTm) {
    try {
      ...
      if (this.tariffHistory && this.tariffHistory.currentTm) {
        const lastTm = new Date(this.tariffHistory.currentTm);
        const pInt = this.priceInterval || 60;
        const lastBoundary = new Date(lastTm);
        lastBoundary.setUTCMinutes(Math.floor(lastTm.getUTCMinutes() / pInt) * pInt, 0, 0);
        const currentBoundary = new Date(currentTm);
        currentBoundary.setUTCMinutes(Math.floor(currentBoundary.getUTCMinutes() / pInt) * pInt, 0, 0);
  ...
  getActiveTariff(reading, tariff, exportTariff) {
    const s = this.getSettings();
    const tType = s && s.tariff_type ? s.tariff_type : 'import';

    if (tType === 'export') return exportTariff;
    if (tType === 'import') return tariff;

    // dynamic: Determine if we are exporting based on realtime measure_power
    if (this.lastMeasure && typeof this.lastMeasure.value === 'number') {
      const isSolar = this.driver.id === 'solar';
      const isExporting = isSolar ? this.lastMeasure.value > 0 : this.lastMeasure.value < 0;
      if (isExporting) return exportTariff;
    }
    return tariff;
  }
