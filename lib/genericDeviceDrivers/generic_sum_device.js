...
class SumMeterDevice extends Device {
  ...
  // would have added so far this month/year, capped at the money actually booked (a device paired
  // mid-year has not been charged the earlier months).
  estimateFixedMoney() {
    const { markup_hour: hour = 0, markup_day: day = 0, markup_month: month = 0 } = this.getSettings();
    const nowLocal = TimeHelpers.toLocalDate(new Date(), this.timeZone);
    const startOfYear = new Date(nowLocal.getFullYear(), 0, 1);
    const dayOfMonth = nowLocal.getDate();
    const dayOfYear = Math.floor((nowLocal - startOfYear) / (24 * 60 * 60 * 1000)) + 1;
    const hoursThisMonth = ((dayOfMonth - 1) * 24) + nowLocal.getHours() + 1;
    const hoursThisYear = ((dayOfYear - 1) * 24) + nowLocal.getHours() + 1;
    const cap = (value, money) => Math.max(0, Math.min(value, Number(money) || 0));
    return {
      month: cap(month + (day * dayOfMonth) + (hour * hoursThisMonth), this.meterMoney && this.meterMoney.month),
      year: cap((month * (nowLocal.getMonth() + 1)) + (day * dayOfYear) + (hour * hoursThisYear), this.meterMoney && this.meterMoney.year),
    };
  }
