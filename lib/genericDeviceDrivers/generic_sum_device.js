...
class SumMeterDevice extends Device {
  ...
  async updateMoney({ ...reading }, { ...periods }) {
    ...
    // Safeguard: if exportTariff is somehow lost or identically equal to undefined, fallback safely.
  ...
  async checkMinMax(val, reading) {
    ...
    if (wattMax === null || val > wattMax) {
      this.lastMinMax.wattMax = val;
      changed = true;
    }
    if (wattMin === null || val < wattMin) {
      this.lastMinMax.wattMin = val;
      changed = true;
    }
