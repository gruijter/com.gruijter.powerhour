...
// How long hot-path bookkeeping may sit in memory before being persisted to the device store.
// setStoreValue() crosses a process boundary and is awaited inside the serialized reading-queue
// drain loop, so writing per reading makes every tick wait on an IPC round-trip - and a listener
// -driven meter ticks several times a minute (once per Wh of register movement). The state behind
// these writes only needs to survive a restart, not be durable to the second; the battery,
// evCharger and solar devices already accept a 15-minute window for exactly this reason
// (see e.g. drivers/battery/device.js#handleUpdateMeter()). One minute is the conservative end of
// that same trade-off: a hard crash or power cut can cost at most this much bookkeeping.
const STORE_FLUSH_MS = 60 * 1000;

class SumMeterDevice extends Device {

  static get STORE_FLUSH_MS() {
    return STORE_FLUSH_MS;
  }
  ...
  // Persist hot-path bookkeeping that STORE_FLUSH_MS throttling may still be holding in memory.
  // Called on a graceful shutdown; a subclass with throttled state of its own overrides this and
  // calls super. Never allowed to reject - it runs while Homey is already tearing the device down.
  async flushStore() {
    if (this.lastMinMax) {
      this.lastMinMaxSaveTm = Date.now();
  ...
  async onUninit() {
    ...
    this.sessionId = null;
    await this.flushStore().catch((err) => this.error(err));
    this.stopPolling();
    this.destroyListeners();
    let delay = 1500;
  ...
  onAdded() {
    ...
    if (this.driver.updateDeviceTariff) {
      this.homey.setTimeout(() => {
        if (!this.isDestroyed) this.driver.updateDeviceTariff(this).catch((err) => this.error(err));
      }, 3000);
    }
  }
  ...
  async onSettings({ newSettings, changedKeys }) {
    ...
    if (this.meterMoney) {
      ...
      // An edited month/year total replaces the fixed costs booked in it: a reset (0) holds none,
      // any other value is taken to hold what the markups added so far (never more than a positive total).
      if (changedKeys.includes('meter_money_this_month') || changedKeys.includes('meter_money_this_year')) {
        const estimate = this.estimateFixedMoney();
        const reseed = (value, est) => {
          const v = Number(value) || 0;
          if (v === 0) return 0;
          return v > 0 ? Math.min(est, v) : est;
        };
        const fixed = { ...(this.fixedMoney || { month: 0, year: 0 }) };
        if (changedKeys.includes('meter_money_this_month')) fixed.month = reseed(money.month, estimate.month);
        if (changedKeys.includes('meter_money_this_year')) fixed.year = reseed(money.year, estimate.year);
        this.fixedMoney = fixed;
        await this.setStoreValue('fixedMoney', fixed).catch((err) => this.error(err));
      }
    ...
    if (this.lastReadingMonth && this.lastReadingYear) {
      if (changedKeys.includes('start_date')) {
        ...
        const now = new Date();
        const nowLocal = TimeHelpers.toLocalDate(now, this.timeZone);
    ...
    if (this.tariffHistory) {
      if (changedKeys.includes('tariff')) {
        ...
        // Without an update group the setting is the only price source, also for export
        // (tariff_type 'export' reads currentExport). With a group, the broadcast owns it.
        if (!newSettings.tariff_update_group) this.tariffHistory.currentExport = newSettings.tariff;
    ...
    if (changedKeys.includes('tariff_update_group')) {
      this.driver.updateDeviceTariff(this, newSettings.tariff_update_group).catch((err) => this.error(err));
    }
  ...
  startPolling(interval) {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (!this.isDestroyed) {
          this.intervalIdDevicePoll = this.homey.setTimeout(() => {
            poll().catch(this.error);
          }, 1000 * 60 * interval);
        }
      }
    };
  ...
  async initDeviceValues() {
    ...
    // Repair devices whose export tariff got stuck at the value from pairing (issue #357): without
    // an update group the tariff setting is the only price source for import and export alike.
    if (!this.settings.tariff_update_group && typeof this.settings.tariff === 'number'
      && (this.tariffHistory.current !== this.settings.tariff || this.tariffHistory.currentExport !== this.settings.tariff)) {
      this.tariffHistory.current = this.settings.tariff;
      this.tariffHistory.currentExport = this.settings.tariff;
      await this.setStoreValue('tariffHistory', this.tariffHistory);
    }
    ...
    // Migrate an old-shape stored lastMinMax right here, before any capability listener can
    // fire. Don't wait for initFirstReading() - that only runs once the meter path accepts a
    // reading, which may never happen (see checkMeterJump()). See ensureMinMaxShape().
    if (this.lastMinMax && this.lastMinMax.reading && this.ensureMinMaxShape(this.lastMinMax.reading)) {
      this.log(`${this.getName()}: migrated stored lastMinMax to the day/month/year shape`);
      await this.setStoreValue('lastMinMax', this.lastMinMax).catch((err) => this.error(err));
    }
    ...
    // Fixed costs (markup_hour/day/month) included in meterMoney.month/year, kept apart so the
    // _avg capabilities can show the energy price per kWh. Otherwise the monthly fee, booked in
    // full on the 1st, makes the month average meaningless for days (issue #229).
    if (!this.fixedMoney) {
      this.fixedMoney = await this.getStoreValue('fixedMoney');
      if (!this.fixedMoney) {
        this.fixedMoney = this.estimateFixedMoney();
        await this.setStoreValue('fixedMoney', this.fixedMoney).catch((err) => this.error(err));
      }
    }
  }

  // One-time seed for devices that predate fixedMoney tracking: what the current markup settings
  // would have added since the period's first reading (lastReadingMonth/Year: the start_date
  // rollover, or pairing for a device paired mid-period). A rollover reading books a markup of each
  // kind; a pairing reading books none. Not capped at the booked money: a net exporter's money can
  // be negative while the markups were charged.
  estimateFixedMoney() {
    const { markup_hour: hour = 0, markup_day: day = 0, markup_month: month = 0 } = this.getSettings();
    const nowMs = Date.now();
    const nowLocal = TimeHelpers.toLocalDate(new Date(nowMs), this.timeZone);
    const localDayNr = (d) => Date.UTC(d.getFullYear(), d.getMonth(), d.getDate()) / (24 * 60 * 60 * 1000);
    const billingMonthNr = (d) => (d.getFullYear() * 12) + d.getMonth() - (d.getDate() < this.startDay ? 1 : 0);
    const bookedSince = (reading) => {
      const fromMs = reading ? new Date(reading.meterTm).getTime() : NaN;
      if (!Number.isFinite(fromMs) || fromMs > nowMs) return 0;
      const fromLocal = TimeHelpers.toLocalDate(new Date(fromMs), this.timeZone);
      const opened = (fromLocal.getDate() === this.startDay && fromLocal.getHours() === 0) ? 1 : 0;
      const hours = Math.floor(nowMs / 3600000) - Math.floor(fromMs / 3600000) + opened;
      const days = localDayNr(nowLocal) - localDayNr(fromLocal) + opened;
      const months = billingMonthNr(nowLocal) - billingMonthNr(fromLocal) + opened;
      return Math.max(0, (month * months) + (day * days) + (hour * hours));
    };
    return {
      month: bookedSince(this.lastReadingMonth),
      year: bookedSince(this.lastReadingYear),
    };
  }
  ...
  async initFirstReading({ ...reading }) {
    ...
    if (this.ensureMinMaxShape(reading)) {
      await this.setStoreValue('lastMinMax', this.lastMinMax).catch((err) => this.error(err));
    }
    this.initReady = true;
  ...
  async updateTariffHistory(tariff, currentTm, priceInterval, exportTariff, args) {
    try {
      ...
      if (this.tariffHistory && this.tariffHistory.currentTm) {
        ...
        const lastBoundary = MeterHelpers.startOfBlock(lastTm, pInt, this.timeZone);
        const currentBoundary = MeterHelpers.startOfBlock(new Date(currentTm), pInt, this.timeZone);
        crossedBoundary = currentBoundary > lastBoundary;
      }
  ...
  // Detect anomalous jumps in the incoming cumulative meter value, in three tiers:
  // - an IMPOSSIBLE incoming value is rejected outright and can never be confirmed - no domestic
  //   meter reaches it, so however often it repeats it is corruption, not a reading.
  // - an ANOMALY jump - scaled to THIS device's own recent usage, see anomalyCeiling() - is
  //   *withheld* rather than trusted immediately. If the very next reading lands close to this
  //   same new level, it's a genuine step-change (source device/meter replacement, migration)
  //   and every period baseline is re-anchored to it - the same fix already proven for the
  //   battery driver (generic_bat_device.js:handleUpdateMeter()), generalized here for
  ...
  // - a GROSS jump (register corruption, unit mixup) goes through that same confirmation flow
  //   but needs one extra consistent reading before it is trusted. It is NOT rejected outright:
  //   see the deadlock note at the `needed` constant below.
  ...
  async checkMeterJump(value) {
    const GROSS_JUMP = 10000; // register corruption / unit mixup - trusted only after 3 readings
    // kWh/m3 no domestic meter can legitimately reach: physical household registers are 6-7
    // digits (they roll over below 1e7 rather than counting past it), and a software counter
    // accumulating real household energy stays in that same magnitude. This is a judgement
    // call, not a spec'd limit - it was 1e9, which is too generous to catch actual corruption:
    // a device reporting 490754146.14 against a stored 4708.66 (report 029aaa63) passed it.
    // It is the ONLY outright rejection left, so it must stay well above any real meter: a
    // value below it that is genuinely wrong is still caught by the confirmation flow, whereas
    // a real value above it would deadlock the device (see the `needed` note below).
    const IMPOSSIBLE_METER = 1e7;

    // An INCOMING value this large is not a meter reading no matter how often it repeats, so
    // it is rejected outright here - before the confirmation flow below, which could otherwise
    // "confirm" it into a re-anchor.
    if (!Number.isFinite(value) || Math.abs(value) > IMPOSSIBLE_METER) {
      this.pendingAnomaly = null;
      throw Error(`ignoring impossible incoming meter value! ${value}`);
    }
    ...
    if (meterDelta > anomalyJump || meterDelta > GROSS_JUMP) {
      // A jump past GROSS_JUMP is more likely register corruption or a unit mixup than a real
      // step-change, so it has to hold still for one EXTRA reading before it is trusted - but it
      // must remain trustable eventually. Rejecting it outright (as this did before) deadlocks
      // the device permanently: every later, perfectly valid reading keeps measuring >GROSS_JUMP
      // against the stale stored value, so the meter never updates again and - because
      // initFirstReading() sits behind this check in updateMeter() - the one-time lastMinMax
      // migration never runs either. The user's only way out was re-pairing.
      // A flapping source still never confirms: the reading that lands back near the original
      // value has a delta of ~0, which clears pendingAnomaly on its way through.
      const needed = meterDelta > GROSS_JUMP ? 3 : 2;
      const { pendingAnomaly } = this;
      if (pendingAnomaly && Math.abs(value - pendingAnomaly.value) <= anomalyJump) {
        const seen = pendingAnomaly.seen + 1;
        if (seen >= needed) {
          // confirmed by consistent repeat readings: genuine step-change.
          this.pendingAnomaly = null;
          this.log(`${this.getName()}: meter jump confirmed (${meterDelta.toFixed(2)}) by ${seen} consistent readings. Re-anchoring baselines to ${value}.`);
          await this.reanchorBaselines(value);
          return;
        }
        this.pendingAnomaly = { value, seen };
        throw Error(`holding anomalous meter value pending confirmation (${seen}/${needed}): ${value} (prev: ${lastVal})`);
      }
      this.pendingAnomaly = { value, seen: 1 };
      throw Error(`holding anomalous meter value pending confirmation (1/${needed}): ${value} (prev: ${lastVal})`);
    }
  ...
  async reanchorBaselines(meterValue) {
    ...
    await this.reanchorExtraBaselines(meterValue).catch((err) => this.error(err));
  }

  // Hook for a subclass that keeps its OWN period anchors alongside the four lastReading*
  // objects re-anchored above - anchors diffed against something the base class never sees
  // (e.g. the grid driver's separate raw import/export registers and its peak-load anchor).
  // reanchorBaselines() cannot re-anchor those itself: it is handed a single netted meterValue
  // and has no idea what else the subclass is tracking. Left unhooked, a confirmed step-change
  // or a repair onto a different physical meter re-anchors the net baselines while the
  // subclass's anchors keep measuring the NEW meter's registers against the OLD meter's - one
  // enormous bogus delta on the very next tick. Base implementation is a no-op, so every
  // driver without extra anchors behaves exactly as before.
  ...
  async updateMeter(val, { selfPoll = false } = {}) {
    try {
      ...
      // Skipped on a selfPoll: `val` is then our OWN meter_source capability, i.e. a value this
      // transform has ALREADY produced. Running it again computes cumVal = dayStartCumVal + cumVal,
      // so the meter climbs by a full day's start value on every boundary poll, and clobbers
      // lastAbsVal (which tracks the RAW source's reading) with a cumulative one. Before the
      // selfPoll flag existed this was caught downstream by checkMeterJump(); that check is now
      // skipped for a selfPoll, which is correct - measuring our own stored value against itself is
      // meaningless - so the transform itself has to be the thing that stands down. See issue on
      // 'virtual via flow' devices with homey_device_daily_reset enabled.
      if (!selfPoll && this.settings.homey_device_daily_reset) {
      ...
      if (!selfPoll) await this.checkMeterJump(value);
      ...
      // (checkMinMax() and initDeviceValues() now also call ensureMinMaxShape() themselves, so
      // that migration no longer depends on this call site being reached at all - note it sits
      // behind checkMeterJump() above, which can withhold readings.)
  ...
  async updateMeterFromFlow(args) {
    ...
    // A poll carries no new source data - it re-feeds our OWN meter capability purely so the
    // hour/day/month/year rollovers still fire on a boundary. Flag it, so checkMeterJump() is
    // skipped: measuring our own stored value against itself is meaningless, and worse, the
    // resulting zero delta would clear a pending anomaly every hour. A flow pushing a genuine
    // step-change at a comparable rate could then ping-pong at 1/3 forever and never confirm.
    const selfPoll = (value === null);
    if (selfPoll) {
      value = this.getCapabilityValue(this.ds.cmap.meter_source);
      if (value === null) return;
    }
    await this.updateMeter(value, { selfPoll });
  }
  ...
  getBudgets() {
    ...
    const date = new Date();
    const dateLocal = TimeHelpers.toLocalDate(date, this.timeZone);
    const yearLocal = dateLocal.getFullYear();
    const startOfMonth = TimeHelpers.toLocalDate(date, this.timeZone);
  ...
  async updateMoney({ ...reading }, { ...periods }) {
    ...
    const { tariff, exportTariff } = MeterHelpers.getPeriodTariff(this.tariffHistory, priceInterval, reading.meterTm, lastReadingTm, undefined, this.timeZone);
    ...
    if (periods.newHour || periods.newDay || periods.newMonth || periods.newYear) {
      const fixed = { ...(this.fixedMoney || { month: 0, year: 0 }) };
      if (periods.newMonth) fixed.month = 0;
      if (periods.newYear) fixed.year = 0;
      fixed.month += fixedMarkup;
      fixed.year += fixedMarkup;
      this.fixedMoney = fixed;
      await this.setStoreValue('fixedMoney', fixed).catch((err) => this.error(err));
    }
  ...
  // Energy price per kWh: fixed costs excluded (see fixedMoney in initDeviceValues()).
  async updateAvgMoney() {
    const fixed = this.fixedMoney || { month: 0, year: 0 };
    const moneyThisMonth = this.meterMoney.month - fixed.month;
    ...
    const moneyThisYear = this.meterMoney.year - fixed.year;
  ...
  async updateMeasureMinMax({ ...reading }) {
    ...
    if (this.capabilityInstances && (this.capabilityInstances.measurePowerRealtime || this.capabilityInstances.measurePower)) {
      ...
      // The in-memory update above is unconditional - checkMinMax()'s two realtime callers read
      // .reading straight off it - but the PERSIST is throttled: this branch used to write the
      // whole lastMinMax object on every single reading, for every device with a realtime
      // measure_power listener (grid, solar, power), which was the app's single most frequent
      // store write. A stale persisted copy costs nothing but a rollover check running against a
      // reading up to STORE_FLUSH_MS old on the first tick after a restart. checkMinMax()'s own
      // write is left alone - it already only fires when a min/max actually moved.
      const nowTm = Date.now();
      if (!this.lastMinMaxSaveTm || (nowTm - this.lastMinMaxSaveTm) > STORE_FLUSH_MS) {
        this.lastMinMaxSaveTm = nowTm;
        await this.setStoreValue('lastMinMax', this.lastMinMax).catch((err) => this.error(err));
      }
      return;
    }
  ...
  // Creates lastMinMax, or migrates an EXISTING device's stored lastMinMax from the old
  // single-pair shape ({wattMax, wattMin, reading, reset} - no .day/.month/.year) to the three
  // period trackers checkMinMax() needs. Returns true when it changed something, so the caller
  // can persist it.
  // Deliberately callable from several places instead of only from initFirstReading(): that one
  // runs only after checkMeterJump() has accepted a reading, but the three realtime paths
  // (addListeners() x2, pollMeter()) call checkMinMax() directly and only test for `.reading`,
  // which the OLD shape also has. So a device whose meter updates are being rejected - or one
  // that simply gets a capability push before its first poll completes after an upgrade - would
  // otherwise crash in checkMinMax() on every single update, indefinitely.
  ensureMinMaxShape(reading) {
    if (this.lastMinMax && this.lastMinMax.day && this.lastMinMax.month && this.lastMinMax.year) return false;
    this.lastMinMax = {
      reading,
      day: {
