...
class SumMeterDevice extends Device {
  ...
  async onSettings({ newSettings, changedKeys }) {
    ...
    if (this.lastReadingDay && this.lastReadingMonth && this.lastReadingYear) {
      ...
      const validateStart = (raw) => {
        ...
        if (typeof currentMeterValue === 'number' && val > currentMeterValue) throw Error(this.homey.__('error_meter_start_invalid'));
        return val;
      };
  ...
  async updateMeters({ ...reading }, { ...periods }) {
    ...
    // Safety net: with a monotonically increasing source meter, a period total should never go
    // negative. If one does anyway (e.g. a manually-entered meter_*_start setting, or any other
    // way a baseline ended up inconsistent with the live meter), re-anchor that baseline to the
    // current reading immediately and persist it - clamping to 0 without re-anchoring would
    // leave the period stuck reporting 0 for its remainder even after the meter catches back up.
    if (valHour < 0) {
      this.log(`${this.getName()}: negative hour total (${valHour.toFixed(3)}) - re-anchoring baseline`);
    ...
    if (valDay < 0) {
      this.log(`${this.getName()}: negative day total (${valDay.toFixed(3)}) - re-anchoring baseline`);
    ...
    if (valMonth < 0) {
      this.log(`${this.getName()}: negative month total (${valMonth.toFixed(3)}) - re-anchoring baseline`);
    ...
    if (valYear < 0) {
      this.log(`${this.getName()}: negative year total (${valYear.toFixed(3)}) - re-anchoring baseline`);
