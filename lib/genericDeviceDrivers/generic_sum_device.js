...
class SumMeterDevice extends Device {
  ...
  async migrate() {
    try {
      ...
      if (this.driver.id !== 'power' && this.driver.id !== 'solar' && this.settings.use_measure_source) {
  ...
  async setCapability(capability, value) {
    if (this.hasCapability(capability) && value !== undefined) {
      ...
      this.setCapabilityValue(capability, val)
        .catch((error) => {
  ...
  async updateMeasureMinMax({ ...reading }, { ...periods }) {
    ...
    const measureValue = (this.driver.id === 'power' || this.driver.id === 'solar') ? measurePowerAvg : measureWaterAvg;
