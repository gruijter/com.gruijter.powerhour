...
class BatDriver extends Driver {
  ...
  async setPricesDevice(device, overrideGroup) {
    ...
    const feedbackGroup = device.getSettings().feedback_tariff_update_group || 0;
    ...
    let feedbackPricesNextHours = pricesNextHours;
    if (feedbackGroup && this.pricesNextHours[feedbackGroup]) {
      feedbackPricesNextHours = this.pricesNextHours[feedbackGroup];
    }

    this.log('updating prices', deviceName, pricesNextHours[0], pricesNextHoursMarketLength);
    await device.updatePrices([...pricesNextHours], [...feedbackPricesNextHours], pricesNextHoursMarketLength, priceInterval, pricesNextHoursIsForecast);
  }
  ...
  async onPairListDevices() {
    try {
      ...
      keys.forEach((key) => {
        ...
        // 1. Check for Homey battery class (New Standard / Priority)
        ...
        // 2. Check for compatible sourceCapGroup in app sources (Old Way / Fallback)
        if (!found) {
