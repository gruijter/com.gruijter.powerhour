...
class BatDriver extends Driver {
  ...
  async setPricesDevice(device, overrideGroup) {
    ...
    const exportGroup = device.getSettings().export_tariff_update_group || 0;
    ...
    let exportPricesNextHours = pricesNextHours;
    if (exportGroup && this.pricesNextHours[exportGroup]) {
      exportPricesNextHours = this.pricesNextHours[exportGroup];
    }

    this.log('updating prices', deviceName, pricesNextHours[0], pricesNextHoursMarketLength);
    await device.updatePrices([...pricesNextHours], [...exportPricesNextHours], pricesNextHoursMarketLength, priceInterval, pricesNextHoursIsForecast);
  }
