...
class MeterHelpers {
  ...
  static calculateMoney(currentMoney, currentReading, tariff) {
    ...
    const newMoney = {
      day: safeAdd(currentMoney.day, deltaMoney),
      month: safeAdd(currentMoney.month, deltaMoney),
      year: safeAdd(currentMoney.year, deltaMoney),
    ...
    if (currentMoney.hour !== undefined) {
      newMoney.hour = safeAdd(currentMoney.hour, deltaMoney);
   ...
   * @param {number|null} livePower Device's own live measure_power (W)
   * @param {number|null} deltaMeter Difference between current and last meter reading
   * @param {number} tariff Import tariff
   * @param {number} exportTariff Export tariff
   * @param {boolean} isSolar Whether the device is a solar panel
   * @param {boolean} isExportOnlyMeter Whether the source meter strictly measures generation/export
   * @returns {number} Active tariff
   */
  static getActiveTariff(settings, currentGridPower, livePower, deltaMeter, tariff, exportTariff, isSolar = false, isExportOnlyMeter = false) {
    ...
    if (isExportOnlyMeter) {
      return exportTariff;
    }
    if (typeof deltaMeter === 'number' && deltaMeter !== 0) {
      return deltaMeter > 0 ? tariff : exportTariff;
    }
    if (typeof livePower === 'number') {
      const isExporting = isSolar ? livePower > 0 : livePower < 0;
      return isExporting ? exportTariff : tariff;
    }
    return tariff;
  }
}
