...
class SumMeterDriver extends Driver {
  ...
  async onUninit() {
    ...
    if (eventType === 'solar' || eventType === 'grid' || eventType === 'evCharger') eventType = 'power';
    ...
    this.eventListenerHour = () => {
      (async () => {
        try {
          ...
          await Promise.all(devices.map((device) => this.handleBoundaryUpdate(device, false)));
        } catch (error) {
          this.error(error);
        }
      })().catch((error) => {
  ...
  register15mListener() {
    ...
    this.eventListener15m = () => {
      (async () => {
        try {
          const devices = this.getDevices();
          await Promise.all(devices.map((device) => this.handleBoundaryUpdate(device, true)));
        } catch (error) {
          this.error(error);
        }
  ...
  async handleBoundaryUpdate(device, isQuarterly) {
    ...
    if (device.getSettings().wait_for_update) {
      ...
      if (device.lastMoneyReadingTm) {
        const now = new Date();
        const lastTm = new Date(device.lastMoneyReadingTm);
        const currentPeriod = Math.floor(now.getMinutes() / interval);
        const lastPeriod = Math.floor(lastTm.getMinutes() / interval);
        if (now.getHours() === lastTm.getHours() && currentPeriod === lastPeriod) doPoll = false;
      }
    }
    ...
    this.eventListenerTariff = (args) => {
      (async () => {
        try {
          ...
          const exportTariff = args.exportTariff !== undefined && args.exportTariff !== null ? Number(args.exportTariff) : undefined;
          const priceInterval = args.priceInterval || 60;
          ...
          for (const device of devices) {
            if (device.settings && device.settings.tariff_update_group && device.settings.tariff_update_group === group) {
              ...
              device.updateTariffHistory(tariff, currentTm, priceInterval, exportTariff, args);
            }
          }
        } catch (error) {
          this.error(error);
  ...
  updateDeviceTariff(device, overrideGroup) {
    ...
    this.log('updating tariff', deviceName, tariff);
    device.updateTariffHistory(tariff, new Date(), priceInterval, exportTariff, args);
  }
    ...
    try {
      ...
      for (const key of keys) {
        ...
        if (compatibility.found) {
          const device = {
            name: `${homeyDevice.name}_Σ${this.ds.driverId}`,
            data: {
          ...
          if (!(homeyDevice.driverId.includes('com.gruijter.powerhour') // ignore own app devices
            || homeyDevice.driverId === 'homey')) devices.push(device); // ignore homey virtual power device
