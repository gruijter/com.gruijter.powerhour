  ...
  async discoverDevices() {
    try {
      ...
      for (const key of keys) {
        if (this.ds.driverId === 'solar') {
          const deviceClass = allDevices[key].class;
          const { virtualClass } = allDevices[key];
          if (deviceClass !== 'solarpanel' && virtualClass !== 'solarpanel') continue;
        } else if (this.ds.requiredClass) {
          const deviceClass = allDevices[key].class;
          const { virtualClass } = allDevices[key];
        ...
        if (this.ds.driverId === 'solar') {
          found = true;
        } else if (this.ds.originDeviceCapabilities) {
