...
class BatDevice extends Device {
  ...
  async updatePrices(pricesNextHours, exportPricesNextHours, pricesNextHoursMarketLength, priceInterval, pricesNextHoursIsForecast, currency) {
    try {
      ...
      this.tariffHistory = {
        previous: this.tariffHistory ? this.tariffHistory.current : pricesNextHours[0],
        previousExport: this.tariffHistory ? this.tariffHistory.currentExport : exportPricesNextHours[0],
        previousTm: this.tariffHistory ? this.tariffHistory.currentTm : new Date(Date.now() - 3600000),
  ...
  async updateMoney({ ...reading }, { ...periods }) {
    ...
    startOfReadingPeriod.setMinutes(Math.floor(readingDate.getMinutes() / priceInterval) * priceInterval, 0, 0);
    ...
    startOfLastPeriod.setMinutes(Math.floor(lastReadingDate.getMinutes() / priceInterval) * priceInterval, 0, 0);
