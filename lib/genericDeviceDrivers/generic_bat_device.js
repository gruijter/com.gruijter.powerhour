...
class BatDevice extends Device {
  ...
  async onSettings({ newSettings, changedKeys }) {
    ...
    if (changedKeys.includes('tariff_update_group') || changedKeys.includes('export_tariff_update_group')) {
  ...
  async updatePrices(pricesNextHours, exportPricesNextHours, pricesNextHoursMarketLength, priceInterval, pricesNextHoursIsForecast) {
    try {
  ...
  async updateMoney({ ...reading }, { ...periods }) {
    ...
    if (typeof this.currentGridPower === 'number') {
      ...
      tariff = this.currentGridPower > 0 ? purchaseTariff : exportTariff;
    } else {
      // fallback if grid power is unknown
      const deltaMeter = reading.meterValue - this.meterMoney.meterValue;
      tariff = deltaMeter > 0 ? exportTariff : purchaseTariff;
    }
