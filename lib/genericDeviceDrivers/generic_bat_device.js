...
class BatDevice extends Device {
  ...
  async onInit() {
    try {
      ...
      if (!this.flows) this.flows = new BatFlows(this);
      ...
      if (this.currencyChanged) await DeviceMigrator.migrateCurrencyOptions(this, this.getSettings().currency, this.getSettings().decimals, '€', 'kWh');
  ...
  onAdded() {
    ...
    // Fetch the active prices from the DAP driver cache immediately after pairing
    if (this.driver.setPricesDevice) {
      this.homey.setTimeout(() => {
        if (!this.isDestroyed) this.driver.setPricesDevice(this).catch((err) => this.error(err));
      }, 3000);
    }
  }
  ...
  async onSettings({ newSettings, changedKeys }) {
    ...
    if (changedKeys.includes('tariff_update_group')) {
      this.driver.setPricesDevice(this, newSettings.tariff_update_group).catch((err) => this.error(err));
    }
    this.homey.setTimeout(() => {
  ...
  async updatePrices(pricesNextHours, exportPricesNextHours, pricesNextHoursMarketLength, priceInterval, pricesNextHoursIsForecast, currency) {
    try {
      ...
      if (currency && currency !== this.getSettings().currency) {
      ...
      const activeTariff = MeterHelpers.getActiveTariff(
        this.getSettings(),
        this.currentGridPower,
        pricesNextHours[0],
        exportPricesNextHours[0],
      );
      ...
      if (this.tariffHistory && this.tariffHistory.currentTm) {
        ...
        const currentBoundary = MeterHelpers.startOfBlock(new Date(), pInt, this.timeZone);
  ...
  async updateValue(val, cap) {
    try {
      ...
      if (cap === 'power') {
        // Single already-signed vendor capability; invert only if the vendor's own convention
        // is the opposite of the Homey standard (positive = charging), per sourceCapGroups.
        const signedVal = this.sourcePowerInvert ? -val : val;
        await this.updateMeterFromMeasure(signedVal).catch((err) => this.error(err));
      }
      if (cap === 'chargePower') {
        // Non-negative magnitude capability: charging is always positive.
        await this.updateMeterFromMeasure(val).catch((err) => this.error(err));
      }
      if (cap === 'dischargePower') {
        // Non-negative magnitude capability: discharging is always negative.
  ...
  async updateMoney({ ...reading }, { ...periods }) {
    ...
    // The "default" (non-crossed) tariff here is the live pricesNextHours/exportTariff computed
    // above, not tariffHistory.current - they normally match (tariffHistory mirrors
    // pricesNextHours whenever updatePrices() runs) but pricesNextHours has its own fallback to
    // the last-known meter_tariff capability if a DAP broadcast ever nulls the price array.
    ...
    const { tariff, exportTariff: expTariff } = MeterHelpers.getPeriodTariff(
      this.tariffHistory,
      priceInterval,
      reading.meterTm,
      lastReadingTm,
      { tariff: purchaseTariff, exportTariff },
      this.timeZone,
    );
