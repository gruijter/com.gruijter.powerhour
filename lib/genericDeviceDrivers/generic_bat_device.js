...
class BatDevice extends Device {
  ...
  async onAdded() {
    ...
    if (this.driver.setPricesDevice) {
      this.homey.setTimeout(() => {
        if (!this.isDestroyed) this.driver.setPricesDevice(this);
      }, 3000);
    }
  }
  ...
  async updateMoney({ ...reading }, { ...periods }) {
    ...
    this.log(`[VERIFY periodTariff] ${this.getName()} priceInterval=${priceInterval}`,
      `lastReadingTm=${new Date(lastReadingTm).toISOString()} readingTm=${new Date(reading.meterTm).toISOString()}`,
      `default={tariff:${purchaseTariff},exportTariff:${exportTariff}} tariffHistory=${JSON.stringify(this.tariffHistory)}`,
      `-> {tariff:${tariff},expTariff:${expTariff}}`);
