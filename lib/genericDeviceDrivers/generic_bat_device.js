...
class BatDevice extends Device {
    ...
    try {
      ...
      if (this.getSettings().roiEnable) {
        this.homey.setTimeout(async () => {
          await setTimeoutPromise(10000 + (Math.random() * 60000), this).catch((err) => this.error(err));
  ...
  async onSettings({ newSettings, changedKeys }) {
    ...
    if (changedKeys.includes('tariff_update_group') || changedKeys.includes('feedback_tariff_update_group')) {
  ...
  async initDeviceValues() {
    ...
    if (!this.feedbackPricesNextHours) this.feedbackPricesNextHours = await this.getStoreValue('feedbackPricesNextHours');
    if (!this.feedbackPricesNextHours) this.feedbackPricesNextHours = this.pricesNextHours;
    ...
    // init this.soc
    const storedkWh = this.getCapabilityValue('meter_kwh_stored');
    const capacity = this.getSettings().batCapacity || 1; // Prevent division by zero
    ...
    const meterCharging = this.getCapabilityValue('meter_kwh_charging');
    const meterDischarging = this.getCapabilityValue('meter_kwh_discharging');
  ...
  async updatePrices(pricesNextHours, feedbackPricesNextHours, pricesNextHoursMarketLength, priceInterval, pricesNextHoursIsForecast) {
    try {
      if (!pricesNextHours || !pricesNextHours[0]) {
        this.pricesNextHours = null;
        this.feedbackPricesNextHours = null;
        ...
        await this.setStoreValue('feedbackPricesNextHours', null).catch((err) => this.error(err));
      ...
      const pricesChanged = JSON.stringify(pricesNextHours) !== JSON.stringify(this.pricesNextHours) || JSON.stringify(feedbackPricesNextHours) !== JSON.stringify(this.feedbackPricesNextHours);
      ...
      this.feedbackPricesNextHours = feedbackPricesNextHours;
      ...
      await this.setStoreValue('feedbackPricesNextHours', feedbackPricesNextHours).catch((err) => this.error(err));
  ...
  async updateChargeChart() {
    ...
    if (strategy) {
      ...
      const chartNextHours = await getChargeChart(strategy, startHour, this.pricesNextHoursMarketLength, this.getSettings().chargePower, this.getSettings().dischargePower, this.priceInterval);
  ...
  async updateValue(val, cap) {
    try {
      ...
      if (cap === 'meterCharging') {
        ...
        await this.setCapability('meter_kwh_charging', val).catch((err) => this.error(err));
        await this.processSourceMeters();
      }
      if (cap === 'meterDischarging') {
        this.lastMeterDischarging = val;
        await this.setCapability('meter_kwh_discharging', val).catch((err) => this.error(err));
        await this.processSourceMeters();
      }
    } catch (error) {
      this.error(error);
  ...
  async updateMoney({ ...reading }, { ...periods }) {
    ...
    const feedbackTariff = (this.feedbackPricesNextHours && this.feedbackPricesNextHours[0] !== undefined) ? this.feedbackPricesNextHours[0] : purchaseTariff;

    let tariff = purchaseTariff;
    if (typeof this.currentGridPower === 'number') {
      // > 0 means the main meter is importing from the grid
      tariff = this.currentGridPower > 0 ? purchaseTariff : feedbackTariff;
    } else {
      // fallback if grid power is unknown
      const deltaMeter = reading.meterValue - this.meterMoney.meterValue;
      tariff = deltaMeter > 0 ? feedbackTariff : purchaseTariff;
    }

    if (purchaseTariff !== this.getCapabilityValue('meter_tariff')) await this.setCapability('meter_tariff', purchaseTariff).catch((err) => this.error(err));
