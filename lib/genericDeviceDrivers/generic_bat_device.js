...
class BatDevice extends Device {
  ...
  async processSourceMeters() {
    ...
    const netMeter = charging - discharging;
  ...
  async updateMoney({ ...reading }, { ...periods }) {
    const tariff = (this.pricesNextHours && this.pricesNextHours[0] !== undefined) ? this.pricesNextHours[0] : (this.getCapabilityValue('meter_tariff') || 0);

    if (tariff !== this.getCapabilityValue('meter_tariff')) await this.setCapability('meter_tariff', tariff).catch((err) => this.error(err));
