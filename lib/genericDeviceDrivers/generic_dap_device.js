...
class MyDevice extends Homey.Device {
  ...
  async onSettings({ newSettings, changedKeys }) {
    ...
    const updateAndRestart = async () => {
      const priceSettings = ['variableMarkup', 'fixedMarkup', 'fixedMarkupTOD', 'fixedMarkupWeekend', 'variableMarkupAbsPrice', 'exchangeRate'];
  ...
  async updatePriceCharts() {
    ...
    const chartToday = await getPriceChart(pricesThisDay, periods.todayStart, 999, this.priceInterval, this.timeZone);
    ...
    const pricesTomorrow = PriceCalculator.selectPriceObjects(this.prices, periods.tomorrowStart, periods.tomorrowEnd)
      .map((p) => ({ price: p.muPrice, isForecast: p.isForecast }));
    const chartTomorow = await getPriceChart(pricesTomorrow, periods.tomorrowStart, 999, this.priceInterval, this.timeZone);
    ...
    const pricesNextHours = this.prices
      .filter((hourInfo) => hourInfo.time >= periods.periodStart)
      .map((p) => ({ price: p.muPrice, isForecast: p.isForecast }));
    const chartNextHours = await getPriceChart(pricesNextHours, periods.periodStart, 999, this.priceInterval, this.timeZone);
