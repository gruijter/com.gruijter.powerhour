...
class MyDevice extends Homey.Device {
  ...
  async onInit() {
    try {
      ...
      // Reorder providers to handle market decoupling
      // Prioritize Nordpool for Nordic/Baltic zones
      const nordpoolZones = ['NO1', 'NO2', 'NO3', 'NO4', 'NO5', 'SE1', 'SE2', 'SE3', 'SE4', 'DK1', 'DK2', 'FI', 'EE', 'LV', 'LT'];
      const isNordpoolZone = this.settings.biddingZone && nordpoolZones.some((z) => this.settings.biddingZone.startsWith(z));
      if (isNordpoolZone) {
        providers.sort((a, b) => {
      ...
      const rateLimit = this.dap[0].getRateLimit();
      this.fetchDelay = (Math.random() * rateLimit.maxRandomDelay) + rateLimit.minDelay;
      if (rateLimit.maxRandomDelay > 300000) {
        this.log('Increased fetch delay for', this.dap[0].constructor.name, 'to', Math.round(this.fetchDelay / 1000 / 60), 'minutes');
      }
   ...
   *
   * Behavior:
   * 1. Triggered every hour via 'everyhour_PBTH' event.
   * 2. Applies a random delay (fetchDelay) to spread API load.
   *    - ENTSOE: 30s - 45m
   *    - Others: 30s - 5m
   * 3. Checks if it's too early to fetch tomorrow's prices (Power only, not Gas).
   *    - Before 10:00 Brussels time: Skips fetch if today's prices are valid.
   *    - After 10:00: Proceeds to fetch (potentially for tomorrow).
   * 4. Iterates through providers (EntsoeGruijter -> Nordpool -> Entsoe -> Stekker).
   * 5. Fetches prices for Yesterday, Today, and Tomorrow.
   * 6. Validates fetched prices (consecutive intervals, not older than stored).
   * 7. Retry Logic:
   *    - EntsoeGruijter: If tomorrow's prices missing after 13:00, retry every ~1-5 mins.
   *    - Nordpool: If tomorrow's prices missing, retry every 15 mins.
   *    - General Failure: If all providers fail, retry in 10-15 mins.
   * 8. Price Corrections:
   *    - Fetches every hour (if allowed by time check) to detect updates to already stored prices.
   *    - Triggers flows if prices have changed.
   */
  async fetchPrices() {
    try {
      ...
      if (this.marketPrices && this.marketPrices.length > 0) {
        ...
        // Skip fetching if we have today's prices and it is before 10:00 Brussels time (ENTSO-E publication time)
        const pricesToday = this.marketPrices.filter((p) => new Date(p.time) >= periods.todayStart && new Date(p.time) < periods.tomorrowStart);
        const hasToday = pricesToday.length >= expectedItems && !pricesToday.some((p) => p.isForecast);
        const nowBrussels = new Date(new Date().toLocaleString('en-US', { timeZone: 'Europe/Brussels' }));
        if (this.driver.id !== 'dapg' && hasToday && nowBrussels.getHours() < 10) {
          this.log(this.getName(), 'Skipping fetch: Too early for tomorrow\'s prices.');
          return;
        }
      }
      ...
      const dateEnd = periods.tomorrowEnd; // Always try to fetch for tomorrow
      let success = false;
      ...
      for (let index = 0; index < this.dap.length; index += 1) {
        ...
        if (this.sessionId !== currentSessionId) return;
        const valid = await this.checkPricesValidity(newMarketPrices, periods).catch((err) => {
          if (err.message === 'Fetched prices are older then the stored prices') {
            this.log(err.message);
          } else if (err.message !== 'Unable to fetch prices') {
            this.log(err);
          }
          return false;
        });
        if (valid) {
          // EntsoeGruijter specific: if tomorrow is missing after 13:00, try next provider
          if (index === 0 && this.dap[index].constructor.name === 'ENTSOE_GRUIJTER') {
            const hasTomorrow = newMarketPrices.some((p) => new Date(p.time) >= periods.tomorrowStart);
            // ENTSO-E prices are published around 13:00 CE(S)T. Use Brussels time to handle DST correctly.
            const nowBrussels = new Date(new Date().toLocaleString('en-US', { timeZone: 'Europe/Brussels' }));
            if (!hasTomorrow && nowBrussels.getHours() >= 13) {
              this.log('Tomorrow prices missing from EntsoeGruijter, trying next provider');
              fallbackPrices = newMarketPrices;
              continue;
            }
          }
          ...
          // Nordpool specific: retry every 15 minutes if tomorrow's prices are missing
          if (this.dap[index].constructor.name === 'Nordpool') {
            const hasTomorrow = newMarketPrices.some((p) => new Date(p.time) >= periods.tomorrowStart);
            if (!hasTomorrow) {
              this.log('Tomorrow prices missing from Nordpool, retrying in 15 mins');
              if (this.retryTimeout) this.homey.clearTimeout(this.retryTimeout);
          ...
          this.log(`${this.getName()} Error fetching prices from ${this.dap[index].host}.`);
        }
      }

      if (!success && fallbackPrices) {
        this.log('Using fallback prices from EntsoeGruijter (missing tomorrow)');
        newMarketPrices = fallbackPrices;
        success = true;
        // Schedule retry for EntsoeGruijter
        if (this.retryTimeout) this.homey.clearTimeout(this.retryTimeout);
        const delay = (60 * 1000) + (Math.random() * 4 * 60 * 1000);
        this.retryTimeout = this.homey.setTimeout(() => {
          if (!this.isDestroyed) this.fetchPrices();
        }, delay);
      }
      ...
      if (!success) {
        ...
        this.log(`${this.getName()} All providers failed. Trying again in ${Math.round(retryDelay / 60000)} minutes`);
        if (this.retryTimeout) this.homey.clearTimeout(this.retryTimeout);
        this.retryTimeout = this.homey.setTimeout(() => {
          if (!this.isDestroyed) this.fetchPrices();
        }, retryDelay);
  ...
  async setCapabilitiesAndFlows(options) {
    try {
      ...
      if (periodChanged && Number.isFinite(this.state.priceNow)) {
        ...
        if (app && app.trigger_new_hour) app.trigger_new_hour(this, tokens);
      }
      ...
      if (!options || !options.noTriggers) {
        ...
        if (Number.isFinite(this.state.priceNow)) {
          ...
          if (app && app.trigger_price_highest) app.trigger_price_highest(this, tokens, state);
          if (app && app.trigger_price_highest_before) app.trigger_price_highest_before(this, tokens, state);
          if (app && app.trigger_price_highest_today) app.trigger_price_highest_today(this, tokens, state);
          if (app && app.trigger_price_above_avg) app.trigger_price_above_avg(this, tokens, state);
          if (app && app.trigger_price_highest_avg) app.trigger_price_highest_avg(this, tokens, state);
          if (app && app.trigger_price_lowest) app.trigger_price_lowest(this, tokens, state);
          if (app && app.trigger_price_lowest_before) app.trigger_price_lowest_before(this, tokens, state);
          if (app && app.trigger_price_lowest_today) app.trigger_price_lowest_today(this, tokens, state);
          if (app && app.trigger_price_below_avg) app.trigger_price_below_avg(this, tokens, state);
          if (app && app.trigger_price_lowest_avg) app.trigger_price_lowest_avg(this, tokens, state);
          if (app && app.trigger_price_lowest_avg_before) app.trigger_price_lowest_avg_before(this, tokens, state);
          if (app && app.trigger_price_highest_avg_before) app.trigger_price_highest_avg_before(this, tokens, state);
        }
      }
    } catch (error) {
      this.error(error);
