...
class MyDevice extends Homey.Device {
  ...
  async updatePriceCharts() {
    ...
    const urlToday = await getPriceChart(pricesThisDay, periods.todayStart, 999, this.priceInterval, this.timeZone);
    ...
    this.todayPriceImage.setStream(async (stream) => {
      return imageUrlToStream(urlToday, stream, this);
    });
    ...
    const pricesTomorrow = PriceCalculator.selectPriceObjects(this.prices, periods.tomorrowStart, periods.tomorrowEnd)
      .map((p) => ({ price: p.muPrice, isForecast: p.isForecast }));
    const urlTomorow = await getPriceChart(pricesTomorrow, periods.tomorrowStart, 999, this.priceInterval, this.timeZone);
    ...
    this.tomorrowPriceImage.setStream(async (stream) => {
      return imageUrlToStream(urlTomorow, stream, this);
    });
    ...
    const pricesNextHours = this.prices
      .filter((hourInfo) => hourInfo.time >= periods.periodStart)
      .map((p) => ({ price: p.muPrice, isForecast: p.isForecast }));
    const urlNextHours = await getPriceChart(pricesNextHours, periods.periodStart, 999, this.priceInterval, this.timeZone);
    ...
    this.nextHoursPriceImage.setStream(async (stream) => {
      return imageUrlToStream(urlNextHours, stream, this);
    });
