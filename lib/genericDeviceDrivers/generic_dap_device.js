...
const PriceFetchStrategy = require('../PriceFetchStrategy');
...
class MyDevice extends Homey.Device {
  ...
  async onInit() {
    try {
      ...
      providers.forEach((Provider, index) => {
        if (Provider.name === 'ENTSOE') apiKey = Homey?.env?.ENTSOE_API_KEY;
  ...
  async fetchPrices() {
    try {
      ...
      let providersToUse = this.dap;
      if (this.settings.priceProvider && this.settings.priceProvider !== 'AUTO') {
        const selected = this.dap.filter((p) => p.constructor.name === this.settings.priceProvider);
        if (selected.length > 0) {
          providersToUse = selected;
        } else {
          this.log(`Selected provider ${this.settings.priceProvider} not available for this zone, falling back to AUTO.`);
        }
      }

      const primaryProviderName = providersToUse && providersToUse[0] ? providersToUse[0].constructor.name : null;

      // Check if we should skip fetching because we already have tomorrow's prices from the primary provider
      const skipCheck = PriceFetchStrategy.shouldSkipFetch(this.marketPrices, periods, this.isNordpoolZone, this.pricesProvider, primaryProviderName);
      if (skipCheck.skip) {
      ...
      const newMarketPrices = await PriceFetchStrategy.fetchPrices(providersToUse, periods, resolution, validator, logger, skipCheck.primaryOnly);

      if (this.sessionId !== currentSessionId) return;

      if (!newMarketPrices) {
        this.log('All providers failed to fetch valid prices. Retrying in 15 mins.');
      ...
      // Check if we are missing Tomorrow and need to retry
      ...
      if (!hasTomorrow) {
        ...
        if (hour >= 13) {
          this.log('Missing tomorrow market prices after 13:00. Retrying in 10 mins.');
          this.scheduleRetry(10);
        } else if (hour >= 10 && (this.isNordpoolZone || this.hasNordpool)) {
          this.log('Missing tomorrow market prices (Nordpool available) after 10:00. Retrying in 15 mins.');
          this.scheduleRetry(15);
        }
      }
    } catch (error) {
  ...
  async setCapabilitiesAndFlows(options) {
    try {
      ...
      const targets = isGas ? ['set_tariff_gas_PBTH'] : ['set_tariff_power_PBTH', 'set_tariff_solar_PBTH', 'set_tariff_water_PBTH'];
