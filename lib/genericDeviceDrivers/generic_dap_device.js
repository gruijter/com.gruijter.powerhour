...
class MyDevice extends Homey.Device {
  ...
  async updatePriceCharts() {
    ...
    const chartToday = await getPriceChart(pricesThisDay, periods.todayStart, 999, this.priceInterval, this.timeZone, this.settings.export_show_chart_line);
    ...
    const pricesTomorrow = PriceCalculator.selectPriceObjects(this.prices, periods.tomorrowStart, periods.tomorrowEnd)
      .map((p) => ({ price: p.muPrice, exportPrice: p.exportPrice, isForecast: p.isForecast }));
    const chartTomorow = await getPriceChart(pricesTomorrow, periods.tomorrowStart, 999, this.priceInterval, this.timeZone, this.settings.export_show_chart_line);
    ...
    const pricesNextHours = this.prices
      .filter((hourInfo) => hourInfo.time >= periods.periodStart)
      .map((p) => ({ price: p.muPrice, exportPrice: p.exportPrice, isForecast: p.isForecast }));
    const chartNextHours = await getPriceChart(pricesNextHours, periods.periodStart, 999, this.priceInterval, this.timeZone, this.settings.export_show_chart_line);
